	...
	<div class="page-header">
		<div class="container-fluid">
			<div class="pull-right">
				<button id="clear-cache" data-toggle="tooltip" title="Clear Cache" class="btn btn-danger"><i class="fa fa-chain-broken"> Home Clear Cache </i></button>
			...
			<div class="panel-body">
				<ul id="languagetabs" class="nav nav-tabs" role="tablist">
					...
					<li <?php echo $class; ?> data-value="<?php echo $language['language_id']; ?>">
						<?php $url = $objurl->link('bytao/layerslider/layer', '&group_id='.$group_id.'&lang='.$language['language_id'].'&token=' . $objsession->data['token'], 'SSL'); ?>
				...
				<br>
				<h3><?php echo $objlang->get('List Of Sliders In Group');?>: <a href="<?php echo  $objurl->link('bytao/layerslider', 'id='.$group_id.'&token=' . $objsession->data['token'], 'SSL');?>"><span><?php echo $sliderGroup['title'];?></span></a></h3>
	    	 	<div>
	    		...
	    		<div class="group-sliders clearfix">
	    			<div class="new-slider-item">
		    			<a href="<?php echo  $objurl->link('bytao/layerslider/layer', 'group_id='.$group_id.'&token=' . $objsession->data['token'], 'SSL')?>">
		    			</a>
	    			...
	    			<?php foreach( $sliders as $slider )  { ?>
	    				<div class="slider-item <?php echo ( $slider['id'] == $slider_id ? 'active':'');?>" id="slider_<?php echo $slider['id'];?>">
	    					<a class="image" href="<?php echo  $objurl->link('bytao/layerslider/layer', 'id='.$slider['id'].'&group_id='.$slider['group_id'].'&lang='.$lang.'&token=' . $objsession->data['token'], 'SSL')?>">
	    						<img class="img-responsive" src="<?php echo HTTP_CATALOG."image/".$slider["image"];?>" height="86"/>
	    					</a>
	    					<a  title="<?php echo $objlang->get('text_clone_this');?>" class="slider-clone" href="<?php echo  $objurl->link('bytao/layerslider/copythis', 'id='.$slider['id'].'&group_id='.$slider['group_id'].'&lang='.$lang.'&token=' . $objsession->data['token'], 'SSL')?>"><span>Clone</span></a>
	    					<a  title="<?php echo $objlang->get('text_delete_this');?>" class="slider-delete" href="<?php echo  $objurl->link('bytao/layerslider/deleteslider', 'id='.$slider['id'].'&group_id='.$slider['group_id'].'&lang='.$lang.'&token=' . $objsession->data['token'], 'SSL')?>" onclick="return confirm('<?php echo $objlang->get('text_confirm_delete');?>')"><span>Delete</span></a>

	    					<a class="slider-status<?php if( !$slider['status']) { ?> slider-status-off <?php } ?>" href="<?php echo  $objurl->link('bytao/layerslider/layer', 'id='.$slider['id'].'&group_id='.$slider['group_id'].'&lang='.$lang.'&token=' . $objsession->data['token'], 'SSL')?>"><span>Status</span></a>
				...
				<form action="" method="post" id="slider-editor-form">
					...
					<input name="slider_language_id" id="slider-lamguage_id" type="hidden" value="<?php echo $slider_image;?>">
				</form>
				<div class="clearfix"></div>
...
$('#clear-cache').on('click',function(){
	...
	$.ajax({
			url: 'index.php?route=bytao/banner/clearcache&token=<?php echo $token; ?>',
	        type: 'post',
	        dataType: 'json',
	        beforeSend: function() {
...
$("#btn-update-slider").click( function(){ 
	...
	$.ajax({
		...
		success: function(html) {
			//$('body').append('<div id="dir-image" class="modal">' + html + '</div>');
