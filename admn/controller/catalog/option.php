...
class Option extends \Opencart\System\Engine\Controller {
	...
	public function form(): void {
		...
		$data['new_value'] = $this->url->link('catalog/option.newvalue', 'user_token=' . $this->session->data['user_token']);
		$data['save_value'] = $this->url->link('catalog/option.savevalue', 'user_token=' . $this->session->data['user_token']);
		$data['delete_value'] = $this->url->link('catalog/option.deletevalue', 'user_token=' . $this->session->data['user_token']);
	...
	*/
	...
	public function newvalue(): void {
		...
		if (isset($this->request->post['add'])&& isset($this->request->post['add'])) {
			foreach ($this->request->post['add'] as $language_id => $value) {
				if ((oc_strlen(trim($value['name'])) < 1) || (oc_strlen($value['name']) > 128)) {
					$json['error']['option_value_name_' . $language_id] = $this->language->get('error_option_value');
				}elseif($this->model_catalog_option->isUniq($this->request->post['option_id'],trim($value['name']))){
					$json['error']['option_value_name_' . $language_id] = $this->language->get('error_option_uniq_value');
				}
			}
		}
		...
		if (!$json) {
			$json['option_value_id'] = $this->model_catalog_option->addOptionValue($this->request->post);
	...
	public function savevalue(): void {
		...
		if (isset($this->request->post['add'])) {
			foreach ($this->request->post['add'] as $language_id => $option_value_description) {
				if ((oc_strlen(trim($option_value_description['name'])) < 1) || (oc_strlen($option_value_description['name']) > 128)) {
					$json['error']['option_value_' . $option_value_id . '_' . $language_id] = $this->language->get('error_option_value');
				}
			}
		...
		if (!$json) {
			...
			$this->model_catalog_option->editOptionValue($this->request->post);
			$json['success'] = $this->language->get('text_success');
			//$json['refresh'] = 1;
	...
	public function deletevalue(): void {
		...
		if (isset($this->request->post['option_value_id'])) {
			$product_total = $this->model_catalog_product->getTotalProductsByOptionValueId($this->request->post['option_value_id']);

			if ($product_total) {
				$json['error'] = sprintf($this->language->get('error_product'), $product_total);
			}
		}else{
			$json['error'] = sprintf($this->language->get('error_product'), '-');
		}

		if (!$json) {
			$this->load->model('catalog/option');
			$this->log->write('deleteOptionValue:'.$this->request->post['option_value_id']);
			$this->model_catalog_option->deleteOptionValue($this->request->post['option_value_id']);
			$json['refresh'] = 1;
			$json['success'] = $this->language->get('text_success');
		}
	...
	protected function opValues(array $vData): string {
		$opData = [];
		
		if (isset($vData['option_id'])) {
			$option_values = $this->model_catalog_option->getValueDescriptions($vData['option_id']);
		} else {
			$option_values = [];
		}
		...
		$opData['languages'] = $this->model_localisation_language->getLanguages();
		$opData['option_values'] = [];
		...
		$opData['placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		
		return $this->load->view('catalog/option_value_list', $opData);
	}
