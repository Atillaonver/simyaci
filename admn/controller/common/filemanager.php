...
class FileManager extends \Opencart\System\Engine\Controller {
	...
	private function base():string {
		return 'catalog/'.((isset($this->session->data['store_path']) && $this->session->data['store_path'])?$this->session->data['store_path'].'/':'').((isset($this->session->data['prod'])&& $this->session->data['prod']!='0' )?'products/'.trim($this->session->data['prod']).'/':'');
	}
	...
	public function index(): void {
		...
		if (isset($this->request->get['sbdir'])) {
			$data['sbdir'] = $this->request->get['sbdir'];
		} else {
			$data['sbdir'] = '';
		}
		...
		if (isset($this->request->get['simage'])) {
			$data['simage'] = $this->request->get['simage'];
		} else {
			$data['simage'] = '';
		}
	...
	public function list(): void {
		$this->load->language('common/filemanager');
		$base = DIR_IMAGE . $this->base();

		if (isset($this->request->get['sbdir'])&& $this->request->get['sbdir']!='') {
			$this->request->get['directory']= 'products/'. html_entity_decode($this->request->get['sbdir'], ENT_QUOTES, 'UTF-8'); 
			if (!is_dir($base . html_entity_decode($this->request->get['directory'], ENT_QUOTES, 'UTF-8') )) {
				$this->request->get['directory']= 'products/';
			}
		}
		...
		if ($paths) {
			...
			foreach ( $paths as $path) {
				...
				if (substr($path, 0, strlen($path)) == $path) {
					...
					if (isset($this->request->get['simage'])) {
						$url .= '&simage=' . $this->request->get['simage'];
					}
					...
					if (isset($this->request->get['multi'])) {
						$url .= '&multi=' . $this->request->get['multi'];
					}
					...
					if(!isset($lItem[$name])){
						...
						if (is_file($path) && in_array(substr($path, strrpos($path, '.')), $allowed)) {
							$data['images'][] = [
								'name'  => $name,
								'path'  => 'catalog/'.(isset($this->session->data['store_path'])?$this->session->data['store_path'].'/':'') . oc_substr($path, oc_strlen($base)),
								'href'  => HTTP_CATALOG . 'image/'.$this->base() . oc_substr($path, oc_strlen($base)),
		...
		if (isset($this->request->get['directory'])) {
			$url .= '&directory=' . urlencode(html_entity_decode($this->request->get['directory'], ENT_QUOTES, 'UTF-8'));
			$data['PATHS']= explode('/',$this->request->get['directory']);
		}
	...
	public function upload(): void {
		...
		$data['bPath'] = $this->base();//'catalog/'.(isset($this->session->data['prod'])?'products/'.trim($this->session->data['prod']).'/':((isset($this->session->data['path'])&& $this->session->data['path'])?$this->session->data['path'].'/':''));
		$base = DIR_IMAGE .$data['bPath'];
		...
		if (!$json) {
			...
			foreach ($files as $file) {
				...
				if (is_file($file['tmp_name'])) {
					...
					$name =by_SEO($_name[0]).'.'.by_SEO($_name[1]);
					$filename = preg_replace('[/\\?%*:|"<>]', '', basename(html_entity_decode($name, ENT_QUOTES, 'UTF-8')));
	...
	public function delete(): void {
		...
		$base = DIR_IMAGE . $this->base();//'catalog/'.((isset($this->session->data['path']) && $this->session->data['path'])?$this->session->data['path'].'/':'');
		...
		foreach ($paths as $path) {
			...
			// Check path exists
			if (($path == $base) || (oc_substr(str_replace('\\', '/', realpath($base . $path)) . '/', 0, oc_strlen($base)) != $base)) {
