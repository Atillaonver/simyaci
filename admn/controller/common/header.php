...
class Header extends \Opencart\System\Engine\Controller {
	public function index(): string {
		...
		$data['stylesheet'] = 'view/stylesheet/stylesheet.css?v1';
		$data['uicss'] = 'https://code.jquery.com/ui/1.14.1/themes/base/jquery-ui.css';
		
		//TODO byTAO added 
		$data['bytao'] = 'view/bytao/css/bytao.css?v190';
		...
		if (!isset($this->request->get['user_token']) || !isset($this->session->data['user_token']) || ($this->request->get['user_token'] != $this->session->data['user_token'])) {
			...
			if ($user_info) {
				...
				$user_stores = explode(',',(string)$user_info['stores']);
			...
			// Stores
			$data['stores'] = [];
			if( $uGId == '1' || in_array('0',$user_stores)) {
				$data['stores'][0] = [
			...
			foreach ($results as $result) {
				if( $uGId == '1' || in_array($result['store_id'],$user_stores)) {
					if(isset($result['store_id'])&&$result['store_id']) {
