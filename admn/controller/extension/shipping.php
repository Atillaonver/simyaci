...
class Shipping extends \Opencart\System\Engine\Controller {
		...
		if ($results) {
			foreach ($results as $result) {
				...
				$data['extensions'][] = [
					...
					'status'     => $this->configGet('shipping_' . $code . '_status') ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
					'sort_order' => $this->configGet('shipping_' . $code . '_sort_order'),
	...
	{
		//$this->log->write('ARRAY:'.print_r($this->store_info,TRUE)); $this->configGet('shipping_desi_cost');
		if ($this->store_info) {
