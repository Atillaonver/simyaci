...
class Order extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		$data['export_tab'] = $this->load->controller('bytao/export',['type'=>'order']);
	...
	protected function getList(): string {
		...
		if (isset($this->request->get['filter_all'])) {
			$filter_all = (int)$this->request->get['filter_all'];
		} else {
			$filter_all = '';
		}
		...
		if (isset($this->request->get['filter_order_key'])) {
			$filter_order_key = (int)$this->request->get['filter_order_key'];
		} else {
			$filter_order_key = '';
		}
		...
		if (isset($this->request->get['filter_order_status_id'])) {
			$data['filter_order_status_id'] = $filter_order_status_id = (int)$this->request->get['filter_order_status_id'];
		} else {
			$filter_order_status_id = '';
		}
		...
		if (isset($this->request->get['filter_order_key'])) {
			$url .= '&filter_order_key=' . $this->request->get['filter_order_key'];
		}
		...
		$filter_data = [
			...
			'filter_order_key'        => $filter_order_key,
		...
		foreach ($results as $result) {
			$host = str_replace(':','',$this->getHost($result['request_url'],0));
			$host = str_replace('astonleather','Direct',$host);
			
			$key = $this->getHost($result['request_url'],1);
			
			
			$sended = $this->model_bytao_mail->getMailCountByMail($result['email']);
			
			$coupons = $this->model_bytao_mail->getCampaignMailByMail($result['email'],$result['order_id']);
			$totalTax = $totalTax + (float)$result['tax'];
			$totalTotal = $totalTotal + (float)$result['total'];
			$totalCargo = $totalCargo + (float)$result['cargo'];
			...
			$data['orders'][] = [
				...
				'tax'           	=> $this->currency->format((float)$result['tax'], $result['currency_code'], $result['currency_value']),
				'cargo'           	=> $this->currency->format((float)$result['cargo'], $result['currency_code'], $result['currency_value']),
				...
				'modal'    	=> $this->url->link('sale/order.modal', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['order_id']),
				'view'            => $this->url->link('sale/order.info', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['order_id'] . $url)
			];
		}
		
		$data['total_cargo'] = $this->currency->format((float)$totalCargo, isset($this->session->data['currency'])?$this->session->data['currency']:'TRY');
		$data['total_tax'] 	 = $this->currency->format((float)$totalTax, isset($this->session->data['currency'])?$this->session->data['currency']:'TRY');
		$data['total_total'] = $this->currency->format((float)$totalTotal,isset($this->session->data['currency'])?$this->session->data['currency']:'TRY');
		...
		if (isset($this->request->get['filter_payment_method'])) {
			$url .= '&filter_payment_method=' . (int)$this->request->get['filter_payment_method'];
		}
		if (isset($this->request->get['filter_order_status_id'])) {
			$url .= '&filter_order_status_id=' . (int)$this->request->get['filter_order_status_id'];
		}
		...
		$data['sort_customer_group_id'] = $this->url->link('sale/order.list', 'user_token=' . $this->session->data['user_token'] . '&sort=o.customer_group_id' . $url);
		$data['sort_status'] = $this->url->link('sale/order.list', 'user_token=' . $this->session->data['user_token'] . '&sort=o.order_status' . $url);
		...
		$data['sort_payment_method'] = $this->url->link('sale/order.list', 'user_token=' . $this->session->data['user_token'] . '&sort=o.payment_method' . $url);
		...
		if($filter_all){
			$data['results'] = sprintf($this->language->get('text_all_pagination'),$order_total);
		}else{
	...
	public function info(): void {
		...
		$data['packingslip'] = $this->url->link('sale/order.packingslip', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $order_id);
		...
		$url = new \Opencart\System\Library\Url($order_info['store_url']);
		...
		foreach ($products as $product) {
			...
			$data['order_products'][] = [
				...
				'href'     		   			=> $url->link('product/product','product_id=' . $product['product_id'].'&language='.$order_info['language_code'])
			];
		}
		...
		// Payment Method
		if (!empty($order_info)&& isset($order_info['payment_method']['name'])) {
			$data['payment_method'] = $order_info['payment_method']['name'];
		} else {
			$data['payment_method'] = '';
		}

		if (!empty($order_info) && isset($order_info['payment_method']['code'])) {
		...
		// Shipping method
		if (!empty($order_info)&& isset($order_info['shipping_method']['name'])) {
			$data['shipping_method'] = $order_info['shipping_method']['name'];
		} else {
			$data['shipping_method'] = '';
		}

		if (!empty($order_info)&& isset($order_info['shipping_method']['code'])) {
		...
		if ($fraud_info) {
				...
				$data['tabs'][] = [
					'code'    => 'fraud',
					'title'   => 'Risk:'.$fraud_info['risk_score'],
					'content' => $this->load->view('sale/order_info_risk', $data)
				];
			} else {
				$data['maxmind_id'] = '';
			}	
		...
		$data['text_form'] = !$order_id ? $this->language->get('text_add') : ( isset($data['risk_score'])?sprintf($this->language->get('text_score_title'), $data['order_key'],$data['risk_score']):sprintf($this->language->get('text_edit'), $data['order_key']));
		...
		if (!empty($order_info)) {
			...
			$data['date_added'] = date($this->language->get('datetime_format'), time());
	...
	public function packingslip(): void {
		...
		$data['bootstrap_css'] = 'view/stylesheet/bootstrap.css';
		$data['icons'] = 'view/stylesheet/fonts/fontawesome/css/all.min.css';
		$data['stylesheet'] = 'view/stylesheet/stylesheet.css?v3';
		...
		foreach ($orders as $order_id) {
			...
			// Make sure there is a shipping method
			if ($order_info && $order_info['shipping_method']) {
				$data['store_id'] = $store_id = $order_info['store_id']? $order_info['store_id']:0;
				
				$store_info = $this->model_setting_setting->getSetting('config', $store_id);
				
				
				if ($store_info) {
					$data['store_url'] = isset($store_info['config_url'])?$store_info['config_url']:HTTP_CATALOG;
					$data['store_logo'] = $store_info['config_logo'];
					$data['store_icon'] = $store_info['config_icon'];
					$store_address = $store_info['config_address'];
					$store_email = $store_info['config_email'];
					$store_telephone = $store_info['config_telephone'];
				} else {
					...
					$data['store_icon'] = $this->config->get('config_icon');
					$store_address = $this->config->get('config_address');
					$store_email = $this->config->get('config_email');
					$store_telephone = $this->config->get('config_telephone');
				}
				...
				foreach ($products as $product) {
					...
					if ($product_info) {
						...
						$product_data[] = [
							...
							'price'    	   => $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
							'total'    	   => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']),
							'weight'       => $this->weight->format(($product_info['weight'] + (float)$option_weight) * $product_info['quantity'], $product_info['weight_class_id'], $this->language->get('decimal_point'), $this->language->get('thousand_point'))
						];
					}
				}
				...
				$data['orders'][] = [
					...
					'shipping_address' => $shipping_address,
					'payment_address'  => $payment_address,
					'shipping_method'  => isset($order_info['shipping_method']['name'])?$order_info['shipping_method']['name']:'RETAILER/CORP',
					'product'          => $product_data,
					'voucher'          => $voucher_data,
					'total'            => $total_data,
					'comment'          => nl2br($order_info['comment'])
		...
		$this->response->setOutput($this->load->view('sale/order_packingslip', $data));
	}
	...
	public function updatesend(): void {
		...
		$this->load->model('sale/order');
		$this->model_sale_order->updatesend();
	...
	public function update(): void {
		...
		if($order_id){
			...
			$this->load->model('sale/order');
			if($this->request->get['type']=='tracking'){
				$key = 'tracking_no';
				$value = $this->request->post['tracking_no'];
			}
			
			if($this->request->get['type']=='cargo'){
				$key = 'cargo';
				$value = $this->request->post['cargo'];
			}
			
			$this->model_sale_order->updateOrder($key,$value,$order_id);
		}
	...
	public function modal():void {
		...
		$first = isset($this->request->get['first'])?$this->request->get['first']:'0';
		
		$data['AllCoupons'] = $this->model_marketing_coupon->getCoupons(['filter_type_customer' => $first]);
		...
		if(isset($this->request->get['order_id'])){
			...
			if($order_info){
				...
				$mails  = $this->model_bytao_mail->getMailsByMail($order_info['email']);
				...
				foreach ($mails as $mail){
					$list[$mail['date_added']] = '<li><span class="date-added">'.$mail['date_added'].'</span> COUPON - <span  class="subject">'.$mail['subject'].'</span></li>';
				...
				$orders_products = $this->model_bytao_mail->getCustomerProductsforMail($order_info['email']);
				...
				foreach($orders_products as $product){
					...
					$data['product_history'][]=[
						...
						'status'		=> $product['order_status_id'],
		...
		$this->response->setOutput($this->load->view('bytao/mail_modal', $data));
	}
	...
	public function mpreview() {
		...
		if (isset($this->request->get['tId'])) {
			...
			$descriptions = $this->model_bytao_mail->getMail_templateDescriptions($this->request->get['tId']);
			$content_main = $descriptions[1]['description'];
			
			$template = $this->model_bytao_mail->getMail_templateById($this->request->get['tId']);
			...
			$json['body'] = $this->load->view('bytao/mail_template_preview', $data);
		}
	...
	public function msend() {
		...
		$descriptions = $json['descriptions'] = $this->model_bytao_mail->getMail_templateDescriptions($this->request->get['tId']);
	...
	public function send() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') 
		{
			...
			$code_id = isset($this->request->post['coupon_id'])?$this->request->post['coupon_id']:"";
			...
			$coupon_info = $this->model_marketing_coupon->getCoupon($code_id);;
			if($coupon_info){
				$content_main = $descriptions[1]['description'];
				...
				if ($emails) {
					if ($email_total!=1) {
						...
						$json['next'] = str_replace('&amp;', '&', $this->url->link('bytao/mail/send', 'token=' . $this->session->data['token'] . '&total=' . $total, 'SSL'));
						$json['oid']=$Orderid;
					} else {
					...
					if (preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $emails[0])) {
						...
						$mail_id = $this->model_bytao_mail->addMail($emails[0],$subject,$message);
					}
			}
	...
	private function getHost($Address,$type=0) {
		$parseUrl = parse_url(trim($Address));
		$query='';
		if($type){
			
			if(isset($parseUrl['query'])){
				if(is_array($parseUrl['query'])){
					$qs=[];
					foreach($parseUrl['query'] as $q){
						$_q = explode('=',$q);
						if(isset($_q[1])){
							$qs[]= $_q[1];
						}
			...
			if(isset($parseUrl['host'])){
				$query=trim($parseUrl['host'] ? str_replace(['WWW.','www.','.com','.es'],'',$parseUrl['host'])  : array_shift(explode('/', $parseUrl['path'], 2)));
			}else{
				$q = explode('?',$Address);
				$query= str_replace(['WWW.','www.','.com','.es'],'',$q[0]) ;
			}
	   		return $query;	
		}
	}
