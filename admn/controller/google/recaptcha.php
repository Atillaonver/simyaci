<?php
namespace Opencart\Admin\Controller\Google;
class Recaptcha extends \Opencart\System\Engine\Controller {

    private $eName   = 'recaptcha_google';
    private $version = '1.0.0';
	private $cPth = 'google/recaptcha';
	private $C = 'recaptcha';
	private $ID = 'recaptcha_id';
    ...
    public function index(): void {
        ...
        $recaptcha_google_settings = $this->model_setting_setting->getSetting($this->eName, $this->session->data['store_id']);

        foreach ($recaptcha_google_settings as $key => $value) {
            $data[$key] = $value;
        }
    ...
    public function save(): void {
        ...
        if (isset($this->request->post['recaptcha_google_status'])) {
            $this->request->post['recaptcha_google_status'] = 1;
        } else {
            $this->request->post['recaptcha_google_status'] = 0;
        }

        if ($this->request->post['recaptcha_google_status']) {
                if ($this->getStrLen(trim($this->request->post['recaptcha_google_secret_key'])) < 1) {
                    $json['error']['secret-key'] = $this->language->get('error_secret-key');
                }
 				if ($this->getStrLen(trim($this->request->post['recaptcha_google_site_key'])) < 1) {
                    $json['error']['site-key'] = $this->language->get('error_site_Key');
                }
        ...
        if (!$json) {
            ...
            $this->model_setting_setting->editSetting($this->eName, $this->request->post, $this->session->data['store_id']);
