...
class Appointment extends \Opencart\System\Engine\Controller {
	...
	public function index():void {
		...
		$this->document->addStyle('view/javascript/jquery/timepicker/jquery.timepicker.min.css');
		$this->document->addScript('view/javascript/jquery/timepicker/jquery.timepicker.min.js');
		
		$data[$this->Tkn] = $this->session->data[$this->Tkn];
		$this->document->addStyle('view/bytao/css/calendar.css?v=22');
		
		$data['JS'] = $this->load->view($this->cPth.'_js', $data);
	...
	protected function getList(): string  {
		...
		$data['lastM'] = $this->url->link($this->cPth.'.list', $this->Tkn.'=' . $this->session->data[$this->Tkn].'&last_date='.$lastM. $url);
		$data['nextM'] = $this->url->link($this->cPth.'.list', $this->Tkn.'=' . $this->session->data[$this->Tkn].'&next_date='.$nextM. $url);
		...
		return $this->load->view($this->cPth.'_calendar', $data);
	...
	public function addplain(): void {
		$this->getML('ML');
		$json = [];
		if (isset($this->request->post['apptimes'])) {
			$times = explode('|',$this->request->post['apptimes']);
			
			foreach($times as $time){
				$_time = explode('~',$time);
				if(count($_time)){
					$json['success'][]= $this->model->{$this->getFunc('edit')}($_time);
				}
			}
		}
	...
	public function getwidget($lData=[]):void {
		$json= [];
		$cData=[];
		$this->getML('L');
		...
		$json['view']=$this->load->view($this->cPth.'_widget_form', $cData);
		$json['ope']='.';
