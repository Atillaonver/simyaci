...
class Sectioner extends \Opencart\System\Engine\Controller {
	
	private $version = '1.0.0';
	private $cPth = 'bytao/sectioner';
	private $C = 'sectioner';
	private $ID = 'sectioner_id';
	...
	protected function getList() {
		...
		$placeholder = $this->model_tool_image->resize('no_image.png', 100, 50);
		
		foreach ($results as $result) {
			if (is_file(DIR_IMAGE . html_entity_decode($result['image'], ENT_QUOTES, 'UTF-8'))) {
				$thumb = $this->model_tool_image->resize(html_entity_decode($result['image'], ENT_QUOTES, 'UTF-8'), 200, 100);
			} else {
				$thumb = $placeholder;
			}
		...
		$data['sort_title'] = $this->url->link($this->cPth, $this->Tkn.'=' . $this->session->data[$this->Tkn] . '&sort=ad.title' . $url);
		$data['sort_sort_order'] = $this->url->link($this->cPth, $this->Tkn.'=' . $this->session->data[$this->Tkn] . '&sort=a.sort_order' . $url);
	...
	public function form() {
		...
		if (is_file(DIR_IMAGE . html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8'))) {
			$data['thumb'] = $this->model_tool_image->resize(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8'), 200, 100);
		} else {
