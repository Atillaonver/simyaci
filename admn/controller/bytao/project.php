...
class Project extends \Opencart\System\Engine\Controller {
	...
	protected function getSetting() {
		...
		$data['projectsetting'] =	$this->config->get('ctrl_project_opt');
	...
	public function form() {
		...
		if (isset($this->request->post['project_category_id'])) {
			...
			$data['project_category_id'] = $item_info['project_category_id'];
			$data['cat_path'] = $this->model->{$this->getFunc('get','CatPath')}($item_info['project_category_id']);
		} else {
			$data['project_category_id'] = 0;
		}
		...
		$data['project_seo_url'] = [];
		$store_id = $data['store_id'] = (int)$this->session->data['store_id'] ;

		if (isset($this->request->get[$this->ID])) {
			$data['project_seo_url'] = $this->model->{$this->getFunc('get','SeoUrls')}($this->request->get[$this->ID]);
		}
    ...
    public function setform(){
		...
		if (!$json) {
			$this->load->model('bytao/common');
			$this->model_bytao_common->setSetting('ctrl_project', $this->request->post);
