...
class ControllerBytaoRevslider extends Controller {
	private $error = array(); 
	...
	$this->language->load('bytao/revslider');
...
//end revsslider custom code
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('bytao/revslider', $token_style.'=' . $token, true),
			'separator' => ' :: '
		);
// start language
...
// end language
		...
		$data['action'] = $this->url->link('bytao/revslider', $token_style.'=' . $token, true);
		
		$data['cancel'] = $this->url->link('common/dashboard', $token_style.'=' . $token . '&type=module', true);
		
		$data['modules'] = array();

		if (isset($this->request->post['revslider_module'])) {
			$data['modules'] = $this->request->post['revslider_module'];
		} elseif ($this->config->get('revslider_module')) { 
			$data['modules'] = $this->config->get('revslider_module');
		}	
...
//start revsslider custom code
                $base_url = $this->url->link('bytao/revslider', $token_style.'=' . $token, true);
...
//end revsslider custom code
		...
		$data['footer'] = $this->load->controller('common/footer');
                ...
                $version = VERSION;
                   ...
                   $this->response->setOutput($this->load->view('bytao/revslider', $data));
		
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'bytao/revslider')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		if (isset($this->request->post['revslider_module'])) {
			foreach ($this->request->post['revslider_module'] as $key => $value) {
	...
	public function install(){
		$this->load->model('bytao/revslider');
		$this->model_bytao_revslider->setup();
		$ABSPATH = get_url();
	...
	public function uninstall(){
		$this->load->model('bytao/revslider');
		$this->model_bytao_revslider->remove();
	}
