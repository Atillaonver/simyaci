<?php
class ControllerBytaoSlayt extends Controller {
	private $error = array();
	private $version = '1.0.0';
	private $cPth = 'bytao/slayt';
	private $C = 'slayt';
	private $ID = 'slayt_id';
	...
	public function index() {   
			...
			if( $this->request->post['action_mode'] == 'module-only' ) {
				...
				unset($this->request->post['action_mode']);
			 	...
			 	if (!isset($this->request->get['module_id'])) {
					...
					$this->response->redirect($this->url->link($this->cPth, 'token=' . $this->session->data['token'], 'SSL'));
				} else {
					$this->model_extension_module->editModule($this->request->get['module_id'], $this->request->post);
					$this->response->redirect($this->url->link($this->cPth, 'token=' . $this->session->data['token'].'&module_id='.$this->request->get['module_id'], 'SSL'));
				}
				...
				$this->response->redirect($this->url->link($this->cPth, $id.'token=' . $this->session->data['token'], 'SSL'));
			}
		...
		if( isset($this->request->get['msg_idone'])  ){
  		...
  		$this->mdata['breadcrumbs'] = array();
   		...
   		$this->mdata['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link($this->cPth, 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		$this->mdata['text_enabled'] = $this->language->get('text_enabled');
		$this->mdata['text_disabled'] = $this->language->get('text_disabled');
		...
		foreach ($stores as &$store) {
			...
			$store['option'] = $this->url->link($this->cPth, $url.'&token=' . $this->session->data['token'], 'SSL');
		}
		...
		$this->mdata['action'] = $this->url->link($this->cPth, 'token=' . $this->session->data['token'].$url, 'SSL');
	...
	public function layer(){
		...
		$this->mdata['cancel'] = $this->url->link($this->cPth, 'token=' . $this->session->data['token'], 'SSL');		
		$this->mdata['token'] = $this->session->data['token'];
	...
	public function cloneGroupSliders(){
 	...
 	public function import(){
		...
		$done = 0;
 		...
 		$url = $this->url->link($this->cPth, 'token=' . $this->session->data['token'].'&msg_idone='.$done, 'SSL');
	 	$this->response->redirect( $url );
 	}
