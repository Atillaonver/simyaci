...
class Mail extends \Opencart\System\Engine\Controller {
	...
	protected function getList() {
		...
		$mail_template_total = $this->model->{$this->getFunc('getTotal','s')};
		...
		foreach ($results as $result) {
			$data['mail_templates'][] = array(
				...
				'edit'           => $this->url->link($this->cPth.'/edit', $this->Tkn.'=' . $this->session->data[$this->Tkn] . '&mail_template_id=' . $result[$this->ID] . $url, 'SSL'),
				'preview'           => $this->url->link($this->cPth.'/preview', $this->Tkn.'=' . $this->session->data[$this->Tkn] . '&mail_template_id=' . $result[$this->ID] . $url, 'SSL')
			);
		}
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($mail_template_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($mail_template_total - $this->config->get('config_limit_admin'))) ? $mail_template_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $mail_template_total, ceil($mail_template_total / $this->config->get('config_limit_admin')));
		...
		$this->response->setOutput($this->load->view($this->cPth.'_template_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get[$this->ID])) {
			...
			$data['action'] = $this->url->link($this->cPth.'/edit', $this->Tkn.'=' . $this->session->data[$this->Tkn] . '&mail_template_id=' . $this->request->get[$this->ID] . $url, 'SSL');
		}
		...
		if (isset($this->request->get[$this->ID]) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$mail_template_info = $this->model->{$this->getFunc('get')}($this->request->get[$this->ID]);
		}
		...
		if (isset($this->request->post['mail_template_description'])) {
			$data['mail_template_description'] = $this->request->post['mail_template_description'];
		} elseif (isset($this->request->get[$this->ID])) {
			$data['mail_template_description'] = $this->model->{$this->getFunc('get','Descriptions')}($this->request->get[$this->ID]);
		} else {
			$data['mail_template_description'] = array();
		}
		...
		if (isset($this->request->post['mail_template_store'])) {
			$data['mail_template_store'] = $this->request->post['mail_template_store'];
		} elseif (isset($this->request->get[$this->ID])) {
			$data['mail_template_store'] = $this->model->{$this->getFunc('get','Stores')}($this->request->get[$this->ID]);
		} else {
			$data['mail_template_store'] = array(0);
		}
		...
		if (isset($this->request->post['status'])) {
			...
			$data['status'] = $mail_template_info['status'];
		} else {
			$data['status'] = true;
		}
		...
		if (isset($this->request->post['type'])) {
			...
			$data['type'] = $mail_template_info['type'];
		} else {
			$data['type'] = 0;
		}
		...
		if (isset($this->request->post['code'])) {
			...
			$data['code'] = $mail_template_info['code'];
		} else {
			$data['code'] = "";
		}
		...
		$this->response->setOutput($this->load->view($this->cPth.'_template_form.tpl', $data));
	}
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $mail_template_id) {
			if ($this->config->get('config_account_id') == $mail_template_id) {
				$this->error['warning'] = $this->language->get('error_account');
			}

			if ($this->config->get('config_checkout_id') == $mail_template_id) {
				$this->error['warning'] = $this->language->get('error_checkout');
			}

			if ($this->config->get('config_affiliate_id') == $mail_template_id) {
				$this->error['warning'] = $this->language->get('error_affiliate');
			}

			if ($this->config->get('config_return_id') == $mail_template_id) {
				$this->error['warning'] = $this->language->get('error_return');
			}

			$store_total = $this->model_setting_store->getTotalStoresByInformationId($mail_template_id);
	...
	public function preview() {
		...
		if (isset($this->request->get[$this->ID])) {
			$this->load->model($this->cPth);
			$descriptions = $this->model->{$this->getFunc('getMail_templateDescriptions')}($this->request->get[$this->ID]);
			$data['mail_content']=html_entity_decode($descriptions[1]['description'], ENT_QUOTES, 'UTF-8');
	...
	public function deletehistory() {
		...
		if (isset($this->request->post['selected'])) {
			foreach ($this->request->post['selected'] as $mail_history_id) {
				$this->model->{$this->getFunc('deleteMail')}($mail_history_id);
			}
	...
	protected function getHistoryList() {
		...
		$mail_template_total = $this->model->{$this->getFunc('getTotalMails')}();

		$results = $this->model->{$this->getFunc('getMails')}($filter_data);
	...
	public function historypreview() {
		...
		if (isset($this->request->get[$this->ID])) {
			$this->load->model($this->cPth);
			$mail = $this->model->{$this->getFunc('getMail')}($this->request->get['mail_history_id']);
			...
			if ($emails) {
				...
				if (preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $emails[0])) {
						...
						*/
						...
						$mail_id = $this->model->{$this->getFunc('addMail')}($emails[0],$subject,$message);
					}
			}
	...
	public function sendby() {
		...
		if (isset($this->request->get['oid'])) {
			...
			$mails  = $this->model->{$this->getFunc('get','sByMail')}($order_info['email']);
	...
	public function couponsend(){
		$this->getML('ML');
		$json = [];
		//$this->model->{$this->getFunc('coupon')}();
		...
		if($product_id && $coupon_id && $mail){
			...
			curl_setopt($curl, CURLOPT_FORBID_REUSE, false);
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($curl, CURLOPT_URL, $store_url . 'index.php?route=bytao/mailer.coupon&bytao=sender');
			curl_setopt($curl, CURLOPT_POST, true);
			curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query($api_info));

			$nJson = curl_exec($curl);
			
			$json['sendedTo']=print_r($nJson,true);
			if (!$nJson) 
			{
