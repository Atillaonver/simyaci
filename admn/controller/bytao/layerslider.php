...
class Layerslider extends \Opencart\System\Engine\Controller {
	...
	public function save(): void {
		...
		if(!isset($json['error'])){
			...
			$json['layerslider_group_id']	=	$groupId;
			
			if($this->request->post['layerslider_group_id'] == 0 ){
	...
	public function form(): void {
		...
		if(isset($this->request->get['layerslider_group_id'])){
			$layersliderGroupId = $this->request->get['layerslider_group_id'];
			
		} else{
			$layersliderGroupId = 0;
		}
		
		$data['layerslider_group_id'] = $layersliderGroupId;
		
		$item_info = $this->model->{$this->getFunc('get','Group')}($layersliderGroupId);
		...
		$data['slider_defaults'] = $this->layerslider_default();
		...
		$type_contents =  isset($css['css'])? css_process($css['css'],(isset($setting['config_url'])?$setting['config_url']:HTTP_CATALOG).'cdn/'):'';
		...
		$sliderGroup =$this->model->{$this->getFunc('get','Group')}( $layersliderGroupId );
		if( !$sliderGroup ){
			$this->response->redirect( $this->url->link($this->cPth, $this->Tkn.'='. $this->session->data[$this->Tkn], 'SSL') );
		}

		$data['sliderGroups'] = $this->model->{$this->getFunc('getList','SliderGroups')}();

		$data['sliderGroup']  = $item_info; 
		$data['sliderHeight'] = (int) $item_info['params']['height'];
		$data['sliderWidth']  = (int) $item_info['params']['width'];  
		...
		foreach($languages as $language){
			$slides =[];
			$slides = $this->model->{$this->getFunc('get','SlidersByGroupId')}( $layersliderGroupId, $language['language_id'] );
			if($slides){
				foreach($slides as $slide){
					...
					$data['slides'][$language['language_id']][] = [
						'title' => $slide['name'],
						'layerslider_id' => $slide['layerslider_id'],
		...
		$data['group_id'] = $layersliderGroupId;
		...
		$default = [
			...
			'slider_thumbnail' => '',
			'slider_slot' =>'7',	
			'slider_image'   => 'catalog/tools/back900x900a.jpg',
			'layer_id'  => '',
			'slider_title' => '',
			'slider-position' => '',
			'slider-layout' => '',
		...
		$slider = $this->model->{$this->getFunc('get','SliderById')}( 0 ); 
		...
		if( $params['slider_thumbnail'] ){
			$data['slider_thumbnail'] =  $this->model_tool_image->resize(  $params['slider_thumbnail'], 
				$sliderGroup['params']['thumbnail_width'], $sliderGroup['params']['thumbnail_height'] );
		}else{
			$data['slider_thumbnail'] = '';
		}
		...
		$data['layers'] = $layers;
		$data['slider_id']  = 0;
		$data['slider_image'] = isset($slider['image'])?$slider['image']:'';  
		$data['slider_image_src'] = URL_IMAGE.'catalog/tools/back900x900a.jpg';
		
		
		$data['placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		$data['fonts'] = json_decode(file_get_contents(DIR_OPENCART.'cdn/fonts/google.json'),true);

		$this->document->addStyle('view/bytao/css/layerslider.css?v=31');
	...
	protected function getList(): string {
		...
		${$this->C.'_total'} = $this->model->{$this->getFunc('getTotal','Groups')}($filter_data);
		...
		foreach($results as $result){
			...
			$data['layersliders'][] = [
				'layerslider_id' => $result['layerslider_group_id'],
				'name'       => $result['name'],
				...
				'edit'       => $this->url->link($this->cPth.'.form', $this->Tkn.'='. $this->session->data[$this->Tkn] . '&layerslider_group_id=' . $result['layerslider_group_id'] , true)
		];
		}
	...
	private function layerslider_default():array {
		$defaults=[];
		$this->getML('ML');
	...
	public function font():void {
		$json = [];
		if(isset($this->request->get['fnt'])){
			$fnt = $this->request->get['fnt'];
			$fonts = json_decode(file_get_contents(DIR_OPENCART.'cdn/fonts/google.json'),true);
			$json['font']=[];
			foreach($fonts['items'] as $font){
				if($font['family']==$fnt){
					$json['font']['subsets']=$font['subsets'];
					$json['font']['variants']=$font['variants'];
				}
			}
		}
		$this->response->addHeader('Content-Type: application/json');
		...
		if(isset($this->request->post['t'])){
			...
			switch($this->request->post['t']){
				case 's'://slider
					...
					{
						if(isset($this->request->post['layerslider_id'])){
							$layerslider_id= $this->request->post['layerslider_id'];
							
							$placeholder = $this->model_tool_image->resize('no_image.jpg', 100, 100);
							$slider = $this->model->{$this->getFunc('get')}($layerslider_id);
							
							if(isset($slider['image'])){
								$thumb = $this->model_tool_image->resize($slider['image'], 100, 100);
								$default = [
									'slider_id'  =>  $layerslider_id,
									'slider_title' => '',
									'slider_link' => '',
									'slider_usevideo' => '0',
									'slider_videoid' => '',
								...
								$layers = [];
								$layerdata = []; 	
								$slide = array_merge( $default, $slider ); 
								$layers = $this->model->{$this->getFunc('get','Layer')}($layerslider_id);
								foreach($layers as $layer)
								{
									$layerParam = unserialize($layer['params']);
									$this->log->write('Array:'.print_r($layerParam,TRUE));
								...
								$params = array_merge( $default, $params ); 
								$params['slider_id'] = $layerslider_id;
								$json['params'] = $params; 
								$json['layers'] = $layerdata; 
								
							}else{
								/*
								$slider = $this->model->{$this->getFunc('get','LayerId')}($layerslider_id);
								
								$params = $slider['params'] ? unserialize( $slider['params'] ) : [];
								$params['params'] = $slider['params'];	
								$thumb = $this->model_tool_image->resize($params['slider_image'], 100, 100);
								$default = [
									'slider_id'  =>  0,
									'slider_title' => '',
									'slider_link' => '',
				...
				case 'G'://Group
					...
					if(isset($this->request->post['layerslider_group_id'])){
						$layersliderGroupId = $this->request->post['layerslider_group_id'];
						$this->load->model('bytao/common');
						...
						foreach($languages as $language){
							...
							if($slides){
								...
								foreach($slides as $slide){
									$laparam =  unserialize($slide['params']);
									if (isset($laparam['id'])|| isset($slide['layerslider_id'])){
										$tData['sliders'] [] = [
											'title' => $laparam['slider_title'],
											'layerslider_id' => isset($slide['layerslider_id'])?$slide['layerslider_id']:$slide['layer_id'],
				...
				case 'U':// Upgrade
					{
					$placeholder = $this->model_tool_image->resize('no_image.jpg', 100, 100);
					$sql = "SELECT * FROM `oc_layerslider`";
					$slides = $this->db->query($sql);
					foreach($slides->rows as $slide){
						
							$group_id = ($slide['layerslider_id']==14)?6:($slide['layerslider_id']==15?7:$slide['layerslider_id']);
							$ssql = "SELECT * FROM `oc_layerslider_layer`  WHERE group_id='".(int)$slide['layerslider_id'] ."' ORDER BY position";
							$layers = $this->db->query($ssql);
						
							foreach($layers->rows as $_layer){
								$params =isset($_layer['params'])?unserialize( $_layer['params'] ) : [];
								$params['slider_group_id'] = $group_id;
								$params['slider_title'] = $_layer['title'];
								$default = [
										'name' => $_layer['title'],
										'group_id' => $group_id,
								...
								//$this->log->write('$layerParams:'.print_r($layerParams,TRUE));
								foreach($layerParams as $_layers){
									if(is_array($_layers)){
										foreach($_layers as $sort_order => $layer){
												$layerdata = [
													"layer_id"			=> "",
													"layer_name"		=> isset($layer['layer_name'])?$layer['layer_name']:'',
													"layer_type"		=> isset($layer['layer_type'])?$layer['layer_type']:'text',
													"layer_caption"		=> isset($layer['layer_caption'])? addslashes( str_replace("'",'"',html_entity_decode( $layer['layer_caption'] , ENT_QUOTES, 'UTF-8')) ):'',
													"layer_content"		=> isset($layer['layer_content'])? addslashes( str_replace("'",'"',html_entity_decode( $layer['layer_content'] , ENT_QUOTES, 'UTF-8')) ):'',
													"layer_image"		=> ($layer['layer_type']=='image')?html_entity_decode( $layer['layer_content'] , ENT_QUOTES, 'UTF-8'):"",
													...
													"layer_link"		=> isset($layer['layer_link'])?$layer['layer_link']:"",
													"layer_linkstatus"	=> isset($layer['layer_linkstatus'])?$layer['layer_linkstatus']:"1",
													"layer_font"		=> isset($layer['layer_font'])?$layer['layer_font']:[],
													"layer_fontweight"	=> isset($layer['layer_fontweight'])?$layer['layer_fontweight']:"",
													"layer_fontsize"	=> isset($layer['layer_fontsize'])?$layer['layer_fontsize']:"",
													"layer_fontcolor"	=> isset($layer['layer_fontcolor'])?$layer['layer_fontcolor']:"",  //#000
													"layer_lineheight"	=> isset($layer['layer_lineheight'])?$layer['layer_lineheight']:"", // px - em vs
												...
												$lquery = "INSERT INTO ".DB_PREFIX . "layerslider_layer ( `";
												$ltmp = [];
												$lvals = [];
	...
	public function lyclone():void {
		...
		if( isset($this->request->get['layer_id']) ){
			...
			if(!isset($json['error']) && isset($slider['layerslider_id'])){
				$this->load->model('tool/image');
				if($slider['image']){
				...
				$json['newlayer'] = $slider['layerslider_id'];
			}
		}
	...
	public function prepos():void{
		...
		$layerslider_group_id = isset($this->request->get['layerslider_group_id'])?$this->request->get['layerslider_group_id']:0;
		...
		$this->load->model('catalog/product');
		$this->load->model('catalog/category');
		$_slide= $this->model->{$this->getFunc('get','GroupImage')}($layerslider_group_id);
	...
	public function savedata ():void {
		...
		if( $this->request->post ){
			...
			$languageId = $this->request->post['slider_language_id'];
			unset( $this->request->post['slider_language_id'] );
			...
			if(!isset($json['error'])){
				...
				$data = [
					...
					'name'   	   => $this->request->post['slider_title'],
					'slider_id'	   => $this->request->post['slider_id'],
				...
				$json['slider_id']	=	$sliderId;
				$json['slider_title']=$this->request->post['slider_title'];
				$json['slider_status']=$this->request->post['slider_status'];
				
				if($this->request->post['slider_image']){
					$json['thumbnail']    = $this->model_tool_image->resize($this->request->post['slider_image'], 200, $th);
					if($json['slider_status'] && $this->request->post['slider_image']){
						$this->model->{$this->getFunc('update','GroupImage')}( $this->request->post['slider_group_id'],$this->request->post['slider_image'] );
					}
				}
				
				if($this->request->post['slider_id'] ==0 ){
	...
	public function getwidget(array $ndata = []) {
		...
		foreach ($results as $result) {
			if($result['status']){
				$data['items'][] = [
					'item_id' 	=> $result['layerslider_group_id'],
					'title'     => $result['name'],
				];
			}
		}
	...
	public function groupautocomplete():void {
		$json = [];
		...
		if(isset($this->request->get['filter_name'])){
			...
			foreach($results as $result){
				...
				$json[] = [
					'item_id' => $result['layerslider_group_id'],
					'item_name'       => strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8')),
				];
			}
		}
