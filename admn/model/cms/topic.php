...
class Topic extends \Opencart\System\Engine\Model {
	...
	public function addTopic(array $data): int {
		...
		foreach ($data['topic_seo_url'] as $language_id => $keyword) {
				$this->model_design_seo_url->addSeoUrl('topic_id', $topic_id, $keyword, $store_id, $language_id,0,'','cms/topic');
		}
	...
	public function editTopic(int $topic_id, array $data): void {
		...
		foreach ($data['topic_seo_url'] as $language_id => $keyword) {
				$this->model_design_seo_url->addSeoUrl('topic_id', $topic_id, $keyword, $store_id, $language_id,0,'cms/blog');
		}
	...
	public function addDescription(int $topic_id, int $language_id, array $data): void {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "topic_description` SET `topic_id` = '" . (int)$topic_id . "', `language_id` = '" . (int)$language_id . "', `image` = '" . $this->db->escape((string)$data['image']) . "', `name` = '" . $this->db->escape($data['name']) . "', `description` = '" . $this->db->escape(by_text_move($data['description'],true,URL_IMAGE)) . "', `meta_title` = '" . $this->db->escape($data['meta_title']) . "', `meta_description` = '" . $this->db->escape($data['meta_description']) . "', `meta_keyword` = '" . $this->db->escape($data['meta_keyword']) . "'");
	}
