...
class Product extends \Opencart\System\Engine\Model {
	...
	public function addProduct(array $data): int {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "product` SET `master_id` = '" . (int)$data['master_id'] . "', `model` = '" . $this->db->escape((string)$data['model']) . "', `sku` = '" . $this->db->escape((string)$data['sku']) . "', `upc` = '" . $this->db->escape((string)$data['upc']) . "', `ean` = '" . $this->db->escape((string)$data['ean']) . "', `jan` = '" . $this->db->escape((string)$data['jan']) . "', `isbn` = '" . $this->db->escape((string)$data['isbn']) . "', `mpn` = '" . $this->db->escape((string)$data['mpn']) . "', `location` = '" . $this->db->escape((string)$data['location']) . "', `variant` = '" . $this->db->escape(!empty($data['variant']) ? json_encode($data['variant']) : '') . "', `override` = '" . $this->db->escape(!empty($data['override']) ? json_encode($data['override']) : '') . "', `quantity` = '" . (int)$data['quantity'] . "', `minimum` = '" . (int)$data['minimum'] . "', `subtract` = '" . (isset($data['subtract']) ? (bool)$data['subtract'] : 0) . "', `stock_status_id` = '" . (int)$data['stock_status_id'] . "', `date_available` = '" . $this->db->escape((string)$data['date_available']) . "', `manufacturer_id` = '" . (int)$data['manufacturer_id'] . "', `shipping` = '" . (isset($data['shipping']) ? (bool)$data['shipping'] : 0) . "', `price` = '" . (float)$data['price'] . "',`sale_price` = '" . (float)$data['sale_price'] . "',`whole_sale_price` = '" . (float)$data['whole_sale_price'] . "',`cost_price` = '" . (float)$data['cost_price'] . "',`retail_price` = '" . (float)$data['retail_price'] . "', `our_price` = '" . (float)$data['our_price'] . "', `points` = '" . (int)$data['points'] . "', `weight` = '" . (float)$data['weight'] . "', `weight_class_id` = '" . (int)$data['weight_class_id'] . "', `length` = '" . (float)$data['length'] . "', `width` = '" . (float)$data['width'] . "', `height` = '" . (float)$data['height'] . "', `length_class_id` = '" . (int)$data['length_class_id'] . "',`measurement_id` = '" . (int)$data['measurement_id'] . "',`productcare_id` = '" . (int)$data['productcare_id'] . "',`material_id` = '" . (int)$data['material_id'] . "',`size_chart_id` = '" . (int)$data['size_chart_id'] . "', `status` = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "', `tax_class_id` = '" . (int)$data['tax_class_id'] . "',`mpage` = '" . (int)$data['mpage'] . "',`sale` = '" . (int)$data['sale'] . "',`best` = '" . (int)$data['best'] . "',`gift` = '" . (int)$data['gift'] . "',`catalog` = '" . (int)$data['catalog'] . "',`new_arriwals` = '" . (int)$data['new_arriwals'] . "',`clearance` = '" . (int)$data['clearance'] . "',`material` = '" . $this->db->escape((string)$data['material']) . "', `sort_order` = '" . (int)$data['sort_order'] . "', `date_added` = NOW(), `date_modified` = NOW(), `version` ='1'");
				...
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					if (isset($product_option['product_option_value'])) {
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO `" . DB_PREFIX . "product_option_value` SET `product_option_id` = '" . (int)$product_option_id . "', `product_id` = '" . (int)$product_id . "', `option_id` = '" . (int)$product_option['option_id'] . "', `option_value_id` = '" . (int)$product_option_value['option_value_id'] . "', `quantity` = '" . (int)$product_option_value['quantity'] . "', `subtract` = '" . (int)$product_option_value['subtract'] . "', `type` = '" . (int)$product_option_value['type'] . "', `price` = '" . (float)$product_option_value['price'] . "', `price_prefix` = '" . $this->db->escape($product_option_value['price_prefix']) . "', `points` = '" . (int)$product_option_value['points'] . "', `points_prefix` = '" . $this->db->escape($product_option_value['points_prefix']) . "', `weight` = '" . (float)$product_option_value['weight'] . "', `weight_prefix` = '" . $this->db->escape($product_option_value['weight_prefix']) . "',`sort_order` = '" . (int)$product_option_value['sort_order'] . "'");
						}
					}
				} else {
		...
		if (isset($data['product_image'])) {
			foreach ($data['product_image'] as $colorid => $product_images) {
				foreach ($product_images as $product_image) {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_image` SET `product_id` = '" . (int)$product_id . "', `image` = '" . $this->db->escape($product_image['image']) . "',`color_id` = '" .(int)$product_image['color_id'] . "', `sort_order` = '" . (int)$product_image['sort_order'] . "'");
				}
			}
		}
		...
		if (isset($data['product_seo_url'])) {
			...
			foreach ($data['product_seo_url'] as $language_id => $keyword) {
					$this->model_design_seo_url->addSeoUrl('product_id', $product_id, $keyword, $store_id, $language_id,2,'','product/product');
					//$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url` SET `store_id` = '" . (int)$store_id . "', `language_id` = '" . (int)$language_id . "', `key` = 'product_id', `value` = '" . (int)$product_id . "', `keyword` = '" . $this->db->escape($keyword) . "',`title` = '".(isset($data['product_description'][$language_id]['name'])?$data['product_description'][$language_id]['name']:"" )."'");
			}
		}
		...
		if($data['new_arriwals']){
			$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'new_arriwals'");
		}
		
		if($data['mpage']){
			$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'mpage'");
		}
		
		if($data['sale']){
			$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'sale'");
		}
		
		if($data['clearance']){
			$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'clearance'");
		}
		
		if($data['gift']){
			$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'gift'");
		}
		
		if($data['best']){
			$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'best'");
		}
		
		if($data['catalog']){
			$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'catalog'");
		}
	...
	public function editProduct(int $product_id, array $data): void {
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "product` SET `model` = '" . $this->db->escape((string)$data['model']) . "', `sku` = '" . $this->db->escape((string)$data['sku']) . "', `upc` = '" . $this->db->escape((string)$data['upc']) . "', `ean` = '" . $this->db->escape((string)$data['ean']) . "', `jan` = '" . $this->db->escape((string)$data['jan']) . "', `isbn` = '" . $this->db->escape((string)$data['isbn']) . "', `mpn` = '" . $this->db->escape((string)$data['mpn']) . "', `location` = '" . $this->db->escape((string)$data['location']) . "', `variant` = '" . $this->db->escape(!empty($data['variant']) ? json_encode($data['variant']) : '') . "', `override` = '" . $this->db->escape(!empty($data['override']) ? json_encode($data['override']) : '') . "', `quantity` = '" . (int)$data['quantity'] . "', `minimum` = '" . (int)$data['minimum'] . "', `subtract` = '" . (isset($data['subtract']) ? (bool)$data['subtract'] : 0) . "', `stock_status_id` = '" . (int)$data['stock_status_id'] . "', `date_available` = '" . $this->db->escape((string)$data['date_available']) . "', `manufacturer_id` = '" . (int)$data['manufacturer_id'] . "', `shipping` = '" . (isset($data['shipping']) ? (bool)$data['shipping'] : 0) . "', `price` = '" . (float)$data['price'] . "',`sale_price` = '" . (float)(isset($data['sale_price'])?$data['sale_price']:0) . "',`whole_sale_price` = '" . (float)$data['whole_sale_price'] . "',`cost_price` = '" . (float)$data['cost_price'] . "',`retail_price` = '" . (float)$data['retail_price'] . "', `our_price` = '" . (float)$data['our_price'] . "', `points` = '" . (int)$data['points'] . "', `weight` = '" . (float)$data['weight'] . "', `weight_class_id` = '" . (int)$data['weight_class_id'] . "', `length` = '" . (float)$data['length'] . "', `width` = '" . (float)$data['width'] . "', `height` = '" . (float)$data['height'] . "', `length_class_id` = '" . (int)$data['length_class_id'] . "',`measurement_id` = '" . (int)$data['measurement_id'] . "',`productcare_id` = '" . (int)$data['productcare_id'] . "',`material_id` = '" . (int)$data['material_id'] . "',`size_chart_id` = '" . (int)$data['size_chart_id'] . "', `status` = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "', `tax_class_id` = '" . (int)$data['tax_class_id'] . "', `mpage` = '" . (int)$data['mpage'] . "',`sale` = '" . (int)$data['sale'] . "',`best` = '" . (int)$data['best'] . "',`gift` = '" . (int)$data['gift'] . "',`catalog` = '" . (int)$data['catalog'] . "',`new_arriwals` = '" . (int)$data['new_arriwals'] . "',`clearance` = '" . (int)$data['clearance'] . "',`material` = '" . $this->db->escape((string)$data['material']) . "',`sort_order` = '" . (int)$data['sort_order'] . "',`version`=`version`+1,`date_modified` = NOW() WHERE `product_id` = '" . (int)$product_id . "'");
		...
		// Categories
		if (isset($data['product_category']) && $data['product_category_change']) {
			...
			foreach ($data['product_option'] as $product_option) {
				...
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					...
					if (isset($product_option['product_option_value'])) {
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO `" . DB_PREFIX . "product_option_value` SET `product_option_value_id` = '" . (int)$product_option_value['product_option_value_id'] . "', `product_option_id` = '" . (int)$product_option_id . "', `product_id` = '" . (int)$product_id . "', `option_id` = '" . (int)$product_option['option_id'] . "', `option_value_id` = '" . (int)$product_option_value['option_value_id'] . "', `quantity` = '" . (int)$product_option_value['quantity'] . "', `subtract` = '" . (int)$product_option_value['subtract'] . "', `type` = '" . (int)$product_option_value['type'] . "', `price` = '" . (float)$product_option_value['price'] . "', `price_prefix` = '" . $this->db->escape($product_option_value['price_prefix']) . "', `points` = '" . (int)$product_option_value['points'] . "', `points_prefix` = '" . $this->db->escape($product_option_value['points_prefix']) . "', `weight` = '" . (float)$product_option_value['weight'] . "', `weight_prefix` = '" . $this->db->escape($product_option_value['weight_prefix']) . "',`sort_order` = '" . (int)$product_option_value['sort_order'] . "'");
						}
					}
		...
		if (isset($data['product_image'])) {
			...
			foreach ($data['product_image'] as $colorid => $product_images) {
				foreach ($product_images as $iid => $product_image) {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_image` SET `product_id` = '" . (int)$product_id . "', `image` = '" . $this->db->escape($product_image['image']) . "',`color_id` = '" .(int)$product_image['color_id'] . "', `sort_order` = '" . (int)$product_image['sort_order'] . "'");
				}
			}
		}
		...
		if($data['new_arriwals']){
			$minq = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND type='new_arriwals' LIMIT 1");
			if(!$minq->row['product_id']){
				$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'new_arriwals'");
			}
		}else{
			$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'new_arriwals'");
		}
		
		if($data['mpage']){
			$minq = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND type='mpage' LIMIT 1");
			if(!$minq->row['product_id']){
				$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'mpage'");
			}
		}else{
			$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'mpage'");
		}
		
		if($data['sale']){
			$minq = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND type='sale' LIMIT 1");
			if(!$minq->row['product_id']){
				$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'sale'");
			}
		}else{
			$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'sale'");
		}
		
		if($data['clearance']){
			$minq = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND type='clearance' LIMIT 1");
			if(!$minq->row['product_id']){
				$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'clearance'");
			}
		}else{
			$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'clearance'");
		}
		
		if($data['gift']){
			$minq = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND type='gift' LIMIT 1");
			if(!$minq->row['product_id']){
				$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'gift'");
			}
		}else{
			$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'gift'");
		}
		
		if($data['best']){
			$minq = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND type='best' LIMIT 1");
			if(!$minq->row['product_id']){
				$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'best'");
			}
		}else{
			$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'best'");
		}
		
		if($data['catalog']){
			$minq = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND type='catalog' LIMIT 1");
			if(!$minq->row['product_id']){
				$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'catalog'");
			}
		}else{
			$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'catalog'");
		}
	...
	public function getProducts(array $data = []): array {
		...
		$sql = "SELECT * FROM `" . DB_PREFIX . "product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`product_id` = pd.`product_id`) LEFT JOIN `" . DB_PREFIX . "product_to_category` p2c ON (p.`product_id` = p2c.`product_id`) LEFT JOIN `" . DB_PREFIX . "product_to_store` p2s ON (p.`product_id` = p2s.`product_id`) WHERE pd.`language_id` = '" . (int)$language_id. "' AND p2s.store_id='".(int)$store_id."' ";
		...
		if (empty($data['filter_all'])&& (isset($data['start']) || isset($data['limit']))) {
			if ($data['start'] < 0) {
				$data['start'] = 0;
			}
	...
	public function getOptions(int $product_id): array {
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			$product_option_value_query = $this->db->query("SELECT *,pov.sort_order as order_sort FROM `" . DB_PREFIX . "product_option_value` pov LEFT JOIN `" . DB_PREFIX . "option_value` ov ON (pov.`option_value_id` = ov.`option_value_id`) WHERE pov.`product_option_id` = '" . (int)$product_option['product_option_id'] . "' ORDER BY pov.`sort_order` ASC");
			...
			foreach ($product_option_value_query->rows as $product_option_value) {
				$product_option_value_data[] = [
					...
					'sort_order'           	 => $product_option_value['order_sort']
				];
			}
	...
	public function getOptionValue(int $product_id, int $product_option_value_id): array {
	    $language_id  = isset($this->session->data['store_language_id'])?$this->session->data['store_language_id']:$this->config->get('config_language_id');
		$query = $this->db->query("SELECT pov.`option_value_id`, ovd.`name`, pov.`quantity`, pov.`subtract`, pov.`price`, pov.`price_prefix`, pov.`sort_order`, pov.`points`, pov.`points_prefix`, pov.`weight`, pov.`weight_prefix` FROM `" . DB_PREFIX . "product_option_value` pov LEFT JOIN `" . DB_PREFIX . "option_value` ov ON (pov.`option_value_id` = ov.`option_value_id`) LEFT JOIN `" . DB_PREFIX . "option_value_description` ovd ON (ov.`option_value_id` = ovd.`option_value_id`) WHERE pov.`product_id` = '" . (int)$product_id . "' AND pov.`product_option_value_id` = '" . (int)$product_option_value_id . "' AND ovd.`language_id` = '" . (int)$language_id . "'");
	...
	public function getSeoUrls(int $product_id): array {
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE `key` = 'product_id' AND `store_id` = '".(int)$store_id."' AND `value` = '" . (int)$product_id . "'");
	...
	public function updateStatus(int $product_id,string $type ):void{
		$sql = "";
		$qRow = $this->db->query("SELECT '".$this->db->escape((string)$type)."' FROM `" . DB_PREFIX . "product` WHERE product_id='".(int)$product_id."' LIMIT 1");
		
		switch($type){
			case 'mpage':
				if($qRow[$type]){
					$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'mpage'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `mpage` = '0' WHERE product_id='".(int)$product_id."'";
					
				}else{
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'mpage'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `mpage` = '1' WHERE product_id='".(int)$product_id."'";
				}
				break;
			...
			case 'sale':
				if($qRow[$type]){
					$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'sale'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `sale` = '0' WHERE product_id='".(int)$product_id."'";
					
				}else{
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'sale'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `sale` = '1' WHERE product_id='".(int)$product_id."'";
				}
				break;
			...
			case 'gift':
				if($qRow[$type]){
					$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'gift'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `gift` = '0' WHERE product_id='".(int)$product_id."'";
					
				}else{
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'gift'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `gift` = '1' WHERE product_id='".(int)$product_id."'";
				}
				break;
			...
			case 'best':
				if($qRow[$type]){
					$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'best'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `best` = '0' WHERE product_id='".(int)$product_id."'";
					
				}else{
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'best'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `best` = '1' WHERE product_id='".(int)$product_id."'";
				}
				break;
			...
			case 'new_arriwals':
				if($qRow[$type]){
					$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'new_arriwals'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `new_arriwals` = '0' WHERE product_id='".(int)$product_id."'";
					
				}else{
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'new_arriwals'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `new_arriwals` = '1' WHERE product_id='".(int)$product_id."'";
				}
				break;
			...
			case 'clearance':
				if($qRow[$type]){
					$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'clearance'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `clearance` = '0' WHERE product_id='".(int)$product_id."'";
					
				}else{
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'clearance'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `clearance` = '1' WHERE product_id='".(int)$product_id."'";
				}
				break;
			...
			case 'catalog':
				if($qRow[$type]){
					$this->db->query("DELETE FROM `" . DB_PREFIX . "product_to_addcategory` WHERE `product_id` = '" . (int)$product_id . "' AND `type` = 'catalog'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `catalog` = '0' WHERE product_id='".(int)$product_id."'";
					
				}else{
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_addcategory` SET `product_id` = '" . (int)$product_id . "', `type` = 'catalog'");
					$sql = "UPDATE `" . DB_PREFIX . "product` SET `catalog` = '1' WHERE product_id='".(int)$product_id."'";
				}
				break;
	...
	*/
	...
	public function getAddCatProducts(array $data = []): array {
		$store_id=isset($this->session->data['store_id'])?$this->session->data['store_id']:0;
		$language_id  = isset($this->session->data['store_language_id'])?$this->session->data['store_language_id']:$this->config->get('config_language_id');
		
		$sql = "SELECT * FROM `" . DB_PREFIX . "product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`product_id` = pd.`product_id`) LEFT JOIN `" . DB_PREFIX . "product_to_addcategory` p2c ON (p.`product_id` = p2c.`product_id`) LEFT JOIN `" . DB_PREFIX . "product_to_store` p2s ON (p.`product_id` = p2s.`product_id`) WHERE pd.`language_id` = '" . (int)$language_id . "' AND p2s.store_id='".(int)$store_id."' ";
		...
		if (!empty($data['filter_addcategory'])) {
			$sql .= " AND p2c.`type` = '" . $this->db->escape((string)$data['filter_addcategory']) . "'";
		}
