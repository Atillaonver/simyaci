...
class Option extends \Opencart\System\Engine\Model {
	...
	public function getValue(int|NULL $option_value_id): array {
		if($option_value_id){
	...
	public function getValues(int $option_id): array {
		...
		$option_value_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option_value` ov LEFT JOIN `" . DB_PREFIX . "option_value_description` ovd ON (ov.`option_value_id` = ovd.`option_value_id`) WHERE ov.`option_id` = '" . (int)$option_id . "' AND ovd.`language_id` = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ovd.`name`");
	...
	public function getValueDescriptions(int $option_id): array {
		...
		$option_value_query = $this->db->query("SELECT ov.* FROM `" . DB_PREFIX . "option_value` ov LEFT JOIN `" . DB_PREFIX . "option_value_description` ovd ON (ov.`option_value_id` = ovd.`option_value_id`) WHERE ov.`option_id` = '" . (int)$option_id . "' ORDER BY ovd.`name`");
	...
	*/
	...
	public function deleteOptionValue(int $option_value_id): void {
	...
	public function addOptionValue(array $data): int {
		$option_id = $data['option_id'];
		$this->db->query("INSERT INTO `" . DB_PREFIX . "option_value` SET `option_id` = '" . (int)$option_id . "', `image` = '" . $this->db->escape(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . "'");

		$option_value_id = $this->db->getLastId();

		foreach ($data['add'] as $language_id => $option_value_description) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "option_value_description` SET `option_value_id` = '" . (int)$option_value_id . "', `language_id` = '" . (int)$language_id . "', `option_id` = '" . (int)$option_id . "', `name` = '" . $this->db->escape($option_value_description['name']) . "'");
		}
		return $option_value_id;
	}
	
	public function editOptionValue(array $data): void {
		
		$sort_order = $data['sort_order'];
		$option_id = $data['option_id'];
		$option_value_id = $data['option_value_id'];
		
		$this->db->query("UPDATE `" . DB_PREFIX . "option_value` SET `image` = '" . $this->db->escape(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order='".(int)$sort_order."' WHERE `option_value_id` = '" . (int)$option_value_id . "'");
		
		$this->db->query("DELETE FROM `" . DB_PREFIX . "option_value_description` WHERE `option_value_id` = '" . (int)$option_value_id . "' AND `option_id` = '" . (int)$option_id . "'");
	...
	public function isUniq(int $option_id,string $name): bool {
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "option_value_description` WHERE `option_id` = '" . (int)$option_id . "' AND `name` = '" . $this->db->escape($name) . "'");
