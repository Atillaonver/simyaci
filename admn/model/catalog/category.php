...
class Category extends \Opencart\System\Engine\Model {
	...
	public function addCategory(array $data): int {
		...
		foreach ($data['category_description'] as $language_id => $value) {
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "category_description` SET `category_id` = '" . (int)$category_id . "', `language_id` = '" . (int)$language_id . "', `name` = '" . $this->db->escape($value['name']) . "', `description` = '" . $this->db->escape($description) . "', `meta_title` = '" . $this->db->escape($value['meta_title']) . "', `meta_description` = '" . $this->db->escape($value['meta_description']) . "', `meta_keyword` = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
		...
		if (isset($data['category_seo_url'])) {
			foreach ($data['category_seo_url'] as $language_id => $keyword) {
					...
					$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url` SET `store_id` = '" . (int)$store_id . "', `language_id` = '" . (int)$language_id . "', `key` = 'path', `value`= '" . $this->db->escape($path) . "', `keyword` = '" . $this->db->escape($keyword) . "',`title` = '".(isset($data['category_description'][$language_id]['name'])?$this->db->escape($data['category_description'][$language_id]['name']):"" )."',route='product/category'");
			}
		}
	...
	public function editCategory(int $category_id, array $data): void {
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "category` SET `parent_id` = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "',`ctype` = '" . (int)$data['ctype'] . "', `size_chart_id` = '" . (int)$data['size_chart_id'] . "',`material_id` = '" . (int)$data['material_id'] . "',`productcare_id` = '" . (int)$data['productcare_id'] . "',`measurement_id` = '" . (int)$data['measurement_id'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `status` = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "', `date_modified` = NOW() WHERE `category_id` = '" . (int)$category_id . "'");
		...
		if (isset($data['category_seo_url'])) {
			foreach ($data['category_seo_url'] as $language_id => $keyword) {
					...
					$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url` SET `store_id` = '" . (int)$store_id . "', `language_id` = '" . (int)$language_id . "', `key` = 'path', `value` = '" . $this->db->escape($path_new) . "', `keyword` = '" . $this->db->escape($keyword) . "',`title` = '".(isset($data['category_description'][$language_id]['name'])?$data['category_description'][$language_id]['name']:"" )."',route='product/category'");
					// Update sub category seo urls
					if (isset($seo_url_data[$language_id])) {
						$this->db->query("UPDATE `" . DB_PREFIX . "seo_url` SET `value` = CONCAT('" . $this->db->escape($path_new . '_') . "', SUBSTRING(`value`, " . (strlen($path_old . '_') + 1) . ")), `keyword` = CONCAT('" . $this->db->escape($keyword) . "', SUBSTRING(`keyword`, " . (oc_strlen($seo_url_data[$language_id]) + 1) . ")) WHERE `store_id` = '" . (int)$store_id . "' AND `language_id` = '" . (int)$language_id . "' AND `key` = 'path' AND `value` LIKE '" . $this->db->escape($path_old . '_%') . "'");
					}
			}
		}
	...
	public function getCategories(array $data = [] ): array {
		...
		if (!empty($data['filter_status'])) {
			$sql .= " AND c1.`status` = '" . $this->db->escape((string)$data['filter_status']) . "'";
		}
	...
	public function getSeoUrls(int $category_id): array {
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE `key` = 'path' AND `store_id` ='".(int)$store_id ."' AND `value` = '" . $this->db->escape($this->getPath($category_id)) . "'");
	...
	public function getTotalCategories(): int {
		$store_id = isset($this->session->data['store_id'])?$this->session->data['store_id']:0;
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "category` c LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) WHERE c2s.store_id = ".$store_id);
	...
	*/
	...
	public function getRelated(int $category_id): array {
		$related_data = [];
	...
	public function getMainParentCategory($category_id):int {
		$query = $this->db->query("SELECT parent_id AS parent FROM " . DB_PREFIX . "category WHERE category_id = '" . (int)$category_id . "'");
		
		return $query->row['parent'];
	}
	
	public function getCategoryRowOrder(int|string $category_id,string $sale_type=''):string {
	...
	public function setCategoryRowOrder(int $category_id,int $row_order,string $sale_type=''):void {
