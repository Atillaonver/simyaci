...
class Coupon extends \Opencart\System\Engine\Model {
	...
	public function getCoupons(array $data = []): array {
		...
		$sql = "SELECT c.`coupon_id`, c.`name`, c.`code`, c.`discount`,c.`type_customer`,c.`type`, c.`date_start`, c.`date_end`, c.`status` FROM `" . DB_PREFIX . "coupon`c LEFT JOIN `" . DB_PREFIX . "coupon_to_store` c2s ON (c.coupon_id = c2s.coupon_id) WHERE c2s.store_id='".(int)$store_id ."'";
		if(isset($data['filter_type_customer']) && $data['filter_type_customer'] ){
			//$sql.=" AND c.`type_customer`='".$data['filter_type_customer']."' ";
		}
	...
	public function getTotalCoupons(): int {
		...
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "coupon` c LEFT JOIN `" . DB_PREFIX . "coupon_to_store` c2s ON (c.coupon_id = c2s.coupon_id) WHERE c2s.store_id = '".(int)$store_id ."'");
