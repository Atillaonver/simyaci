...
class User extends \Opencart\System\Engine\Model {
	public function addUser(array $data): int {
		...
		if (isset($data['user_store'])) {
			foreach ($data['user_store'] as $store_id) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "user_to_store` SET `user_id` = '" . (int)$category_id . "', `store_id` = '" . (int)$store_id . "'");
			}
		}
	...
	public function getUser(int $user_id): array {
		$query = $this->db->query("SELECT *, (SELECT ug.`name` FROM `" . DB_PREFIX . "user_group` ug WHERE ug.`user_group_id` = u.`user_group_id`) AS user_group,(SELECT GROUP_CONCAT(u2s.`store_id` SEPARATOR ',') FROM `" . DB_PREFIX . "user_to_store` u2s WHERE u2s.`user_id` = u.`user_id`) AS stores FROM `" . DB_PREFIX . "user` u WHERE u.`user_id` = '" . (int)$user_id . "'");
	...
	public function getStores(int $user_id): array {
		$user_store_data = [];
