...
class ModelBytaoSlayt extends Model { 
	...
	public function install(){
		...
		$sql = " SHOW TABLES LIKE '".DB_PREFIX."slaytgroups'";
		$query = $this->db->query( $sql );
		...
		if( count($query->rows) <=0 ){
			...
			if( file_exists($file) ){
				require_once( $file );
		 	    ...
		 	    $result = $sample->installSampleQuery( $this->config->get('config_template'),'slaytlayer', true );
		 	    $result = $sample->installSample( $this->config->get('config_template'),'slaytlayer', true );
			}
		}	
		$query = $this->db->query( $sql );
		if( count($query->rows) <=0 ){ 
	...
	protected function createTables(){
		...
		$sql[] = "
				CREATE TABLE IF NOT EXISTS `".DB_PREFIX."slaytgroups` (
		...
		";
		$sql[] = "
				CREATE TABLE IF NOT EXISTS `".DB_PREFIX."slaytlayers` (
				   	  `id` int(11) NOT NULL AUTO_INCREMENT,
					  `title` varchar(255) NOT NULL,
		...
		$sql[]="ALTER TABLE `".DB_PREFIX."slaytlayers` ADD `language_id` int(11) DEFAULT '1'";
	...
	public function saveSliderGroupData( $data ){
		...
		if( isset($data['id']) && $data['id'] ){
		 	$query = " UPDATE  ". DB_PREFIX . "slaytgroups SET  ";
		 	$tmp = array();
			...
			$this->db->query( $query );
	 		...
	 		$query = "INSERT INTO ".DB_PREFIX . "Slaytgroups ( `";
			$tmp = array();
			$vals = array();
			foreach( $data as $key => $value ){
				$tmp[] = $key;
	...
	public function saveData( $data ){
		 if( isset($data['id']) && $data['id'] ){
		 	$query = " UPDATE  ". DB_PREFIX . "slaytlayers SET  ";
		 	$tmp = array();
		 	foreach( $data as $key => $value ){
				if( $key != "id" ){
					$tmp[] = "`".$key."`='".$this->db->escape($value)."'";
			...
			$this->db->query( $query );
	 		...
	 		$query = "INSERT INTO ".DB_PREFIX . "slaytlayers ( `";
			$tmp = array();
			$vals = array();
			foreach( $data as $key => $value ){
				$tmp[] = $key;
	...
	public function getListSliderGroups( ){
		$query = ' SELECT * FROM '. DB_PREFIX . "Slaytgroups   ";
	...
	public function getSliderById( $id ){
		$query = ' SELECT * FROM '. DB_PREFIX . "slaytlayers   ";
	...
	public function deleteSlider( $id ){
		$query = ' DELETE FROM '. DB_PREFIX . "slaytlayers  WHERE id=".$id;
		$this->db->query( $query );
	}
	public function delete( $id ){
		$query = ' DELETE FROM '. DB_PREFIX . "slaytgroups  WHERE id=".$id;

		$this->db->query( $query );
		$query = ' DELETE FROM '. DB_PREFIX . "slaytlayers  WHERE group_id=".$id;
		$this->db->query( $query );
	...
	public function updatePost( $id , $pos ){
		$sql = 'UPDATE '.DB_PREFIX.'slaytlayers SET `position`='.$pos.' WHERE id='.($id);
		$this->db->query( $sql );
	}
}
