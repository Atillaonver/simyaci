...
class ExportImport extends \Opencart\System\Engine\Model {
	...
	protected function repairSEOKeywords() {
		$paths = $this->getPaths();
		$this->log->write("ExportImport::repairSEOKeywords: paths='".print_r($paths,true)."'");
		...
		$this->log->write("ExportImport::repairSEOKeywords: query->rows='".print_r($query->rows,true)."'");
		$this->log->write("ExportImport::repairSEOKeywords: keywords='".print_r($keywords,true)."'");
...
//		$col -= 1; // we use 1-based, PHPExcel uses 0-based column index, PhpSpreadsheet now uses 1-based column index
	...
	protected function getCategorySEOKeywords( &$languages, $min_id, $max_id ) {
		...
		$sql .= "SUBSTRING_INDEX(`value`,'_',-1)>='".(int)$min_id."' AND ";
		$sql .= "SUBSTRING_INDEX(`value`,'_',-1)<='".(int)$max_id."' ";
		$sql .= "ORDER BY SUBSTRING_INDEX(`value`,'_',-1), store_id, language_id";
		$query = $this->db->query( $sql );
		$seo_keywords = array();
		foreach ($query->rows as $row) {
	...
	protected function curl_get_contents_x($url) {
		$ch = curl_init();
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents_x("https://www.mhccorp.com/index.php?route=information/message&type=tool_export_import_".str_replace('.','_',$this->version)."&language_code=$language_code");
