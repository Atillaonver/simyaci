...
class Layout extends \Opencart\System\Engine\Model {
	public function addLayout(array $data): int {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "layout` SET `name` = '" . $this->db->escape((string)$data['name']) . "',`store_id` =".(int)$store_id);
	...
	public function getLayouts(array $data = []): array {
		...
		$sql = "SELECT * FROM `" . DB_PREFIX . "layout` WHERE store_id=".(int)$store_id;
		// end byTAO
	...
	public function getTotalLayouts(): int {
		...
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "layout` WHERE store_id=".(int)$store_id);
