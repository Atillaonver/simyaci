...
class Banner extends \Opencart\System\Engine\Model {
	public function addBanner(array $data): int {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "banner` SET `name` = '" . $this->db->escape((string)$data['name']) . "', `status` = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "', store_id = '" . (int)$this->session->data['store_id']. "'");
	...
	public function getBanners(array $data = []): array {
		...
		$sql = "SELECT * FROM `" . DB_PREFIX . "banner` WHERE store_id = '" . (int)$storeID. "'";
