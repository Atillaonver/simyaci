...
class Customer extends \Opencart\System\Engine\Model {
	...
	public function getCustomerByEmail(string $email): array {
		...
		$query = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "customer` WHERE LCASE(`email`) = '" . $this->db->escape(oc_strtolower($email)) . "' AND store_id='".(int)$store_id."'");
	...
	public function getCustomers(array $data = []): array {
		...
		$sql .= " WHERE cgd.`language_id` = '" . (int)$this->config->get('config_language_id') . "' AND c.`store_id` = '".$store_id."'";
	...
	public function getTotalCustomers(array $data = [],int $store_id=0,$dash=true): int {
		if($dash){
			$store_id = isset($this->session->data['store_id'])?$this->session->data['store_id']:0;
		}
