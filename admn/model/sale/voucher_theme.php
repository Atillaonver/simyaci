...
class VoucherTheme extends \Opencart\System\Engine\Model {
	public function addVoucherTheme(array $data): int {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "voucher_theme` SET `image` = '" . $this->db->escape((string)$data['image']) . "',store_id='".$store_id."'");
	...
	public function getVoucherThemes(array $data = []): array {
		...
		if ($data) {
			$sql = "SELECT * FROM `" . DB_PREFIX . "voucher_theme` vt LEFT JOIN `" . DB_PREFIX . "voucher_theme_description` vtd ON (vt.`voucher_theme_id` = vtd.`voucher_theme_id`) WHERE vtd.`language_id` = '" . (int)$this->config->get('config_language_id') . "' AND vt.`store_id` = '" . (int)$store_id . "' ORDER BY vtd.`name`";
			...
			if (!$voucher_theme_data) {
				$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "voucher_theme` vt LEFT JOIN `" . DB_PREFIX . "voucher_theme_description` vtd ON (vt.`voucher_theme_id` = vtd.`voucher_theme_id`) WHERE vtd.`language_id` = '" . (int)$this->config->get('config_language_id') . "' AND vt.`store_id` = '" . (int)$store_id . "' ORDER BY vtd.`name`");
	...
	public function getTotalVoucherThemes(): int {
		...
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "voucher_theme` WHERE `store_id` = '" . (int)$store_id . "'");
