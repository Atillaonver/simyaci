...
class Order extends \Opencart\System\Engine\Model {
	...
	public function getOrder(int $order_id): array {
		...
		if ($order_query->num_rows) {
			...
			return [
				...
				'order_key'                => $order_query->row['order_key'],
	...
	public function getOrders(array $data = []): array {
		...
		$sql = "SELECT o.`order_id`,o.`order_key`,o.`device`,o.`customer_id`,o.`request_url`,o.`email`,o.`keyword`,o.`frm`, CONCAT(o.`firstname`, ' ', o.`lastname`) AS customer, (SELECT os.`name` FROM `" . DB_PREFIX . "order_status` os WHERE os.`order_status_id` = o.`order_status_id` AND os.`language_id` = '" . (int)$this->config->get('config_language_id') . "') AS order_status, o.`store_name`, o.`shipping_method`, o.`payment_method`, o.`payment_code`, o.`total`,(SELECT `value` FROM `" . DB_PREFIX . "order_total` WHERE order_id = o.`order_id` AND code ='localtax' LIMIT 1) AS tax,(SELECT `risk_score` FROM `" . DB_PREFIX . "order_fraud` WHERE order_id = o.`order_id` LIMIT 1) AS risk,(SELECT `value` FROM `" . DB_PREFIX . "order_total` WHERE order_id = o.`order_id` AND code ='shipping' LIMIT 1) AS cargo, o.`currency_code`, o.`customer_group_id`, o.`currency_value`, o.`date_added`, o.`date_modified` FROM `" . DB_PREFIX . "order` o";
		...
		if (!empty($data['filter_order_id'])) {
			$sql .= " AND (o.`order_id` = '" . (int)$data['filter_order_id'] . "' OR o.`order_key` = '" . (int)$data['filter_order_id'] . "')";
		}elseif (!empty($data['filter_order_key'])) {
			$sql .= " AND o.`order_key` = '" . (int)$data['filter_order_key'] . "'";
		}
		...
		if (!empty($data['filter_customer_group_id'])) {
			if($data['filter_customer_group_id']=='2'){
	...
	public function getTotalOrders(array $data = [],int $store_id = 0,bool $dash = true): int {
		if($dash){
			$store_id = isset($this->session->data['store_id'])?$this->session->data['store_id']:0;
		}
	...
	public function updateOrder($key,$value,$order_id):void {
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET `".$key."` = '" . $value . "' WHERE `order_id` = '" . (int)$order_id . "'");
	...
	public function getTodayTotalOrders($store_id = 0) {
	...
	public function getTodayTotalCustomers($store_id = 0) {
	...
	public function getOrdersByEmail($email){
		$query = $this->db->query("SELECT o.order_id,o.order_key,o.email,o.telephone, CONCAT(o.firstname, ' ', o.lastname) AS customer,(SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status,o.order_status_id AS order_status, o.shipping_code, o.total, o.currency_code, o.currency_value, o.date_added FROM `" . DB_PREFIX . "order` o WHERE o.email ='" . $email . "' AND o.order_status_id > 0 ORDER BY o.date_added");
