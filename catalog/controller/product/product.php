...
class Product extends \Opencart\System\Engine\Controller {
	...
	public function index(): \Opencart\System\Engine\Action|null {
		...
		if ($product_info) {
			...
			$data['addtocart'] = $this->url->link('checkout/cart.add', 'language=' . $this->config->get('config_language'));
			$data['cartinfo'] = $this->url->link('common/cart.info', 'language=' . $this->config->get('config_language'));
			...
			if (is_file(DIR_IMAGE . html_entity_decode(by_move($product_info['image']), ENT_QUOTES, 'UTF-8'))) {
			...
			$this->log->write([
				...
				'SIMPLE PRICE'  => $data['simple_price'],
			...
			$downloads = [];
			$results = $this->model_catalog_product->getDownloads($product_id);
			foreach ($results as $result) {
				if (is_file(DIR_DOWNLOAD . $result['filename'])) {
					$downloads[] = [
						'name'       => $result['name'],
						'href'       => $this->url->link('product/product.download', 'language=' . $this->config->get('config_language') . '&indir_im=' . $result['download_id'])
					];
				}
			}
			
			$data['downloads'] = $this->load->view('product/download_list',['downloads' => $downloads]);
	...
	public function download(): void
	{
		...
		if (isset($this->request->get['indir_im'])) {
			$download_id = (int)$this->request->get['indir_im'];
		} else {
			$download_id = 0;
		}
		if (isset($this->request->get['brosur'])) {
			$brosur = (int)$this->request->get['brosur'];
		} else {
			$brosur = 0;
		}
		...
		if ($download_info) {
			...
			$product_info = $this->model_catalog_product->getProduct($brosur);
			if ($product_info){
				$file = DIR_DOWNLOAD . $product_info['filename'];
