...
class Bridge extends \Opencart\System\Engine\Controller {
	...
	private $cPth = 'bytao/bridge';
	private $C = 'bridge';
	private $ID = 'bridge_id';
	private $model ;
	...
	public function index(): void {
		...
		$data['send'] = $this->url->link('bytao/bridge.send', 'language=' . $this->config->get('config_language'));
	...
	public function contact(): void {
		...
		if (isset($this->request->post['token'])) {
			...
			if(!$resp['success']){
				$json['error']['captcha'] = sprintf($this->language->get('error_captcha'),$resp['success'],$resp['score'] );
			}
	...
	public function form():void{
		...
		if(isset($this->request->post['sendfor'])){
			if($this->request->post['sendfor']=='newsletter'){
			...
			if($this->request->post['sendfor'] == 'demoreq'){
				...
				if(!isset($json['error'])){
					$send = $this->model->mailSend();
					if($send){
						$json['success'] = $this->language->get('success_demoreq');
					}else{
						$json['error'] = $this->language->get('error_demoreq');
					}	
				}
			}
		}
