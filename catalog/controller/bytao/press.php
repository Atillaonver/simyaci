...
class Press extends \Opencart\System\Engine\Controller {
	...
	public function index():void {
		...
		$data['witems'] = [];
		$data['presses'] =$this->url->link($this->cPth, 'language=' . $this->config->get('config_language'));
		...
		if ($items) {
			...
			foreach ($items as $result) {
				if (is_file(DIR_IMAGE . $result['image'])) {
					$data['witems'][] = [
						'title'  => $result['title'],
						'href'  => $this->url->link($this->cPth.'.get_widget', 'language=' . $this->config->get('config_language').'&press_id='.$result['press_id']),
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $limit,
			'url'   => $this->url->link('bytao/press', 'language=' . $this->config->get('config_language') . $url . '&page={page}')
		]);
		...
		$this->document->addScript('cdn/js/colorbox-master/jquery.colorbox-min.js');
		$this->document->addStyle('cdn/js/colorbox-master/colorbox.css');
		...
		$data['header'] = $this->load->controller('bytao/header',['root'=>'bytao-press']);
	...
	public function getwidget(array $lData):string {
		...
		$total_items = $this->model->{$this->getFunc('getTotal','es')}($filter_data);
		...
		if ($items) {
			...
			foreach ($items as $result) {
				if (is_file(DIR_IMAGE . $result['image'])) {
					$data['items'][] = array(
						...
						'description'  => $result['description'],
						'video'  => $result['video'],
						'thumb' => $this->model_tool_image->resize($result['image'],isset($lData['thumb_width'])?$lData['thumb_width']:570,isset($lData['thumb_height'])?$lData['thumb_height']:400),
					);
				}
			}
			...
			$data['pagination'] = $this->load->controller('common/pagination', [
				...
				'page'  => $page,
				'limit' => $limit,
				'url'   => $this->url->link($this->cPth.'.pagination', 'language=' . $this->config->get('config_language') . $url . '&page={page}')
			]);
	...
	public function ajax(){
		...
		if ($item_info) {
			$this->response->setOutput($this->load->view($this->cPth.'_ajax',$item_info));
		}else{
			return '';
		}
