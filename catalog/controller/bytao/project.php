...
class Project extends \Opencart\System\Engine\Controller {
	...
	public function index() {
		...
		$ITEM = $this->model->{$this->getFunc('get')}($project_id);
		if($ITEM){
			...
			$project_opts =	$this->config->get('ctrl_project_opt');
			
			$lid = $this->config->get('config_language_id');
			$data['opt_titles']=[];
			foreach($project_opts as $project_opt){
				$data['opt_titles'][$project_opt['code']]= $project_opt['opt'][$lid]['name'];
			}
			
			$data['heading_title'] = $ITEM['name'];
			$data['title'] = $ITEM['title'];
			$data['project_opts'] = $ITEM['opt'];
			...
			$data['header'] = $this->load->controller('bytao/header',['root'=>'bytao- project']);
