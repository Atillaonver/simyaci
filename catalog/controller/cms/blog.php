...
class Blog extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		if (isset($this->request->get['search'])) {
			...
			$filter_search = (string)$this->request->post['search'];
		} else {
			$filter_search = '';
		}
		...
		$data['search_action'] =$this->url->link('cms/blog', 'language=' . $this->config->get('config_language') . $url);
		...
		foreach ($results as $result) {
			...
			if ($result['image'] && is_file(DIR_IMAGE . html_entity_decode(by_move($result['image']), ENT_QUOTES, 'UTF-8'))) {
	...
	public function info(): ?\Opencart\System\Engine\Action {
		...
		if ($article_info) {
			...
			if (!empty($article_info['image']) && is_file(DIR_IMAGE . html_entity_decode($article_info['image'], ENT_QUOTES, 'UTF-8'))) {
				$data['thumb'] = $this->model_tool_image->resize($article_info['image'], $this->config->get('config_image_article_width'), $this->config->get('config_image_article_height'));
			} else {
				$data['thumb'] = '';
			}
