...
class Confirm extends \Opencart\System\Engine\Controller {
	...
	public function orderconfirm(array $order=[]): string {
		
		$content = isset($order['content']) ? html_entity_decode($order['content'], ENT_QUOTES, 'UTF-8'):'';
		if ($content && isset($this->session->data['customer']) ) {

			$format = '{firstname} {lastname}' . "\n" . ' {address_1} {address_2} {postcode} {city} {zone} {country}' . "\n" . 'Telefon:{telephone}'. "\n" ;
			
			$format2 = '{firstname} {lastname}';
			...
			$alici ="|ALICI|";
			$content = str_replace($alici, $address , $content);
			...
			$alici ="|urunler|";
			$address = html_entity_decode($this->load->controller('bytao/pdf.products',[$this->session->data['order_id']]) , ENT_QUOTES, 'UTF-8');
			$content = str_replace($alici, $address , $content);
			
			$content = str_replace("|tarih|", "" , $content);
