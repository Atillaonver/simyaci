...
class Register extends \Opencart\System\Engine\Controller {
	...
	public function register(): void {
		...
		if (!$json) {
			...
			$lnam = isset($this->request->post['lastname'])?$this->request->post['lastname']:'';
			
			if ((oc_strlen($lnam) < 1) || (oc_strlen($lnam) > 32)) {
				$json['error']['lastname'] = $this->language->get('error_lastname');
			}else{
				$chars = oc_substr($lnam, oc_strlen($lnam)-3);
				$map = '';
				
				for ($pos = 0; $pos < oc_strlen($chars); $pos++) {
					$char = oc_substr($chars, $pos, 1);
