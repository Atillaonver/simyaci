 ...
 */
 require_once(DIR_SYSTEM . 'library/hg_mailer.php');
class Contact extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		$data['gsm'] = $this->config->get('config_gsm');
		...
		$data['reCaptchaForm'] = $this->load->controller('google/recaptcha.sform');
	...
	public function send(): void {
		...
		if (isset($this->request->post['g-recaptcha-response'])) {
			$reCaptcha = $this->load->controller('google/recaptcha.response', $this->request->post);
			if (!$reCaptcha['success'] || $reCaptcha['score'] < 0.5) {
				$json['error']['warning'] = 'Güvenlik doğrulaması geçilemedi, lütfen tekrar deneyin.';
			}
		} else {
			$json['error']['warning'] = 'Güvenlik anahtarı eksik.';
		}
