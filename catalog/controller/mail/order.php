...
class Order extends \Opencart\System\Engine\Controller {
	...
	public function add(array $order_info, int $order_status_id, string $comment, bool $notify): void {
		...
		$data['store_mail']= $this->model_setting_setting->getValue('config_email', $order_info['store_id']);
		$data['customer_id'] = $order_info['customer_id'];
		$data['link'] = $order_info['store_url'] . 'index.php?route=account/order.info&order_id=' . $order_info['order_id'];
		
		$this->load->model('bytao/pdf');
		$token = $this->model_bytao_pdf->getPdfToken($order_info['order_id']);
		$data['contract_link'] = $token?$this->url->link('bytao/pdf.download','token=' . $token,true):'';
		...
		if ($this->config->get('config_mail_engine')) {
			...
			//$this->log->write('order add:'.print_r($mail_option,TRUE));
	...
	public function edit(array $order_info, int $order_status_id, string $comment, bool $notify): void {
		...
		$this->load->model('bytao/pdf');
		$token = $this->model_bytao_pdf->getPdfToken($order_info['order_id']);
		$data['contract_link'] = $token?$this->url->link('bytao/pdf.download','order_id=' . (int)$order_info['order_id'] . '&token=' . $token,true):'';
		...
		$from = $data['store_mail']= $this->model_setting_setting->getValue('config_email', $order_info['store_id']);
		...
		if ($this->config->get('config_mail_engine')) {
			...
			//$this->log->write('order edit:'.print_r($mail_option,TRUE));
