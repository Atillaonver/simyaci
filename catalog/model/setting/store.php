...
class Store extends \Opencart\System\Engine\Model {
	...
	public function runAs():int {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "setting` WHERE  `store_id` = 0 AND `key` LIKE 'config_run_store_as'");

		return isset($query->row['value'])?$query->row['value']:0;
	}
	public function runerAs():int {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "setting` WHERE  `store_id` = 0 AND `key` LIKE 'config_runer_store_as'");
