...
class Product extends \Opencart\System\Engine\Model {
	...
	public function getProduct(int|string $product_id): array {
		...
		$sql .= " p.`sort_order`,p.`best`,p.`sale`,p.`gift`,p.`clearance`,p.`new_arriwals`,";
		$sql .= " p.`sort_order` FROM `" . DB_PREFIX . "product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`product_id` = pd.`product_id`) LEFT JOIN `" . DB_PREFIX . "product_to_store` p2s ON (p.`product_id` = p2s.`product_id`) LEFT JOIN `" . DB_PREFIX . "manufacturer` m ON (p.`manufacturer_id` = m.`manufacturer_id`) WHERE p.`product_id` = '" . (int)$product_id . "' AND pd.`language_id` = '" . (int)$this->config->get('config_language_id') . "' AND p.`status` = '1' AND p.`date_available` <= NOW() AND p2s.`store_id` = '" . (int)$this->config->get('config_store_id') . "'";
		...
		if ($query->num_rows) {
			...
			if($this->customer->isLogged()&& $this->customer->getGroupId()==2){
				$product_data['price'] = $query->row['whole_sale_price'];
				$product_data['sale_price']=0;
				$product_data['old_price'] =0;
			} else if($query->row['sale_price'] > 0){
				$product_data['old_price'] = $query->row['price'];
				$product_data['price'] = $query->row['sale_price'];
			}else if($query->row['discount']>0) { 
				$product_data['old_price'] = $query->row['price'];
				$product_data['price'] = $query->row['discount'];
			}else if($query->row['special']>0) { 
				$product_data['old_price'] = $query->row['price'];
				$product_data['price'] = $query->row['special'];
			}else{
				$product_data['price'] = $query->row['price'];
				$product_data['old_price'] =0;
			}
			...
			$product_data['description'] = by_text_move($product_data['description'], false,DIR_IMAGE);
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_name'])) {
				...
				foreach ($words as $word) {
					$implode[] = "(LCASE(pd.`name`) LIKE '" . $this->db->escape('%' . oc_strtolower($word) . '%') . "' OR pd.`name` LIKE '" . $this->db->escape('%' . $word . '%') . "')";
				}
			...
			if (!empty($data['filter_name'])) {
				$sql .= " OR LCASE(p.`model`) LIKE '%" . $this->db->escape(oc_strtolower($data['filter_name'])) . "%'";
	...
	public function getOptions(int $product_id): array {
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			if (strpos($product_option['name'], 'Size') !== false) {
				$optionname="Size";
			}elseif (strpos($product_option['name'], 'Colors') !== false) {
				$optionname="Color";
			}else{
				$optionname=$product_option['name'];
			}
	...
	public function getImages($product_id): array {
		if((int)$product_id ){
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_image` WHERE `product_id` = '" . (int)$product_id . "' ORDER BY `color_id`, `sort_order` ASC");
	...
	{
		$query = $this->db->query("SELECT DISTINCT d.`download_id`, dd.`name`, d.`filename` FROM `" . DB_PREFIX . "download` d LEFT JOIN `" . DB_PREFIX . "product_to_download` p2d ON (d.`download_id` = p2d.`download_id`) LEFT JOIN `" . DB_PREFIX . "download_description` dd ON (d.`download_id` = dd.`download_id`) WHERE p2d.`product_id` = '" . (int)$product_id . "' AND dd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getProductCategoriesRelated($categories) {
		...
		foreach($categories AS $category){
				$limit = (int)4/count($categories);
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.category_id='" . (int)$category['related_id'] . "' AND ".$status." AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY RAND() LIMIT ".$limit);
	...
	public function getJobTotalProducts(array $data = []): int {
		$sql = "SELECT COUNT(DISTINCT p.`product_id`) AS total";
		...
		if (!empty($data['filter_job'])) {
			switch($data['filter_job']){
				case 'mpage':$addSql = " AND p.mpage = '1' "; break;
				case 'best':$addSql = " AND p.best = '1' "; break;
				case 'gift':$addSql = " AND p.gift = '1' "; break;
				case 'sale':$addSql = " AND p.sale = '1' "; break;
				case 'clearance':$addSql = " AND p.clearance = '1' "; break;
				case 'newarriwals':$addSql = " AND p.new_arriwals = '1' "; break;
			}
		} 
	...
	public function getJobProducts(array $data = []): array {
	...
	public function getProductCategoryProduct(int $category_id,$limit) {
		$product_data = [];
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.category_id='" . (int)$category_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY RAND() LIMIT ".$limit);
		foreach ($query->rows as $result) {
					$product_data[$result['product_id']] = $this->getProduct($result['product_id']);
		}
