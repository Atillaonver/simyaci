...
class Order extends \Opencart\System\Engine\Model {
	...
	public function addOrder(array $data): int {
		...
		if (isset($data['products'])) {
			//$this->log->write('add Order products:'.print_r($data['products'],TRUE));
			
			foreach ($data['products'] as $product) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "order_product` SET `order_id` = '" . (int)$order_id . "', `product_id` = '" . (int)$product['product_id'] . "', `master_id` = '" . (int)$product['master_id'] . "', `name` = '" . $this->db->escape($product['name']) . "', `image` = '" . $this->db->escape($product['image']) . "', `model` = '" . $this->db->escape($product['model']) . "', `quantity` = '" . (int)$product['quantity'] . "', `price` = '" . (float)$product['price'] . "', `total` = '" . (float)$product['total'] . "', `tax` = '" . (float)$product['tax'] . "', `reward` = '" . (int)$product['reward'] . "'");
	...
	public function editOrder(int $order_id, array $data): void {
		...
		if ($order_info) {
			...
			// Products
			if (isset($data['products'])) {
				//$this->log->write('edit Order products:'.print_r($data['products'],TRUE));
				foreach ($data['products'] as $product) {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "order_product` SET `order_id` = '" . (int)$order_id . "', `product_id` = '" . (int)$product['product_id'] . "', `master_id` = '" . (int)$product['master_id'] . "', `name` = '" . $this->db->escape($product['name']) . "', `image` = '" . $this->db->escape($product['image']) . "', `model` = '" . $this->db->escape($product['model']) . "', `quantity` = '" . (int)$product['quantity'] . "', `price` = '" . (float)$product['price'] . "', `total` = '" . (float)$product['total'] . "', `tax` = '" . (float)$product['tax'] . "', `reward` = '" . (int)$product['reward'] . "'");
