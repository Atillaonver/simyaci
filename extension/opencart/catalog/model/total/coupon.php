		...
		if ($code) {
			...
			if ($status) {
				if($coupon_query->row['uses_customer'] == '0'){
					$SQL = "INSERT INTO `" . DB_PREFIX . "coupon_history` SET `coupon_id` = '" . (int)$coupon_query->row['coupon_id'] . "', `order_id` = '" . (int)$order_info['order_id'] . "', `customer_id` = '" . (int)$order_info['customer_id'] . "', `amount` = '" . (float)$order_total['value'] . "', `date_added` = NOW()";
					
				}else{
					$SQL = "UPDATE `" . DB_PREFIX . "coupon_personal` SET status = '1',order_id = '" . (int)$order_info['order_id'] . "',date_use = NOW() WHERE code = '" . $this->db->escape($coupon_query->row['code']) . "' AND email = '" . $this->db->escape($order_info['email']) . "' AND (date_start < NOW()) AND (date_end > NOW()) AND status = '0'";
