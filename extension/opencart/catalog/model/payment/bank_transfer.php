...
class BankTransfer extends \Opencart\System\Engine\Model {
	public function getMethods(array $address = []): array {
		...
		if ($this->cart->hasSubscription()) {
			...
			$sql="SELECT * FROM `" . DB_PREFIX . "zone_to_geo_zone` WHERE `geo_zone_id` = '" . (int)$this->config->get('payment_bank_transfer_geo_zone_id') . "' AND `country_id` = '" . (int)$address['country_id'] . "' AND `zone_id` = '" . (int)$address['zone_id'] . "'";
			//$query = $this->db->query($sql);

			$this->log->write(' address:'.print_r($sql,true));
