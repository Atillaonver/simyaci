...
class Latest extends \Opencart\Catalog\Model\Catalog\Product {
	...
	public function getLatest(int $limit): array {
		...
		$product_data = $this->cache->get('product.' . md5($sql) , $this->config->get('config_store_id'));
		...
		if (!$product_data) {
			...
			$this->cache->set('product.' . md5($sql), $product_data , $this->config->get('config_store_id'));
		}
