...
class BankTransfer extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		$this->configGet('payment_bank_transfer_geo_zone_id');
		...
		foreach ($languages as $language) {
			$data['payment_bank_transfer_bank'][$language['language_id']] = $this->configGet('payment_bank_transfer_bank_' . $language['language_id']);
		}

		$data['languages'] = $languages;

		$data['payment_bank_transfer_order_status_id'] = $this->configGet('payment_bank_transfer_order_status_id');

		$this->load->model('localisation/order_status');

		$data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();

		$data['payment_bank_transfer_geo_zone_id'] = $this->configGet('payment_bank_transfer_geo_zone_id');

		$this->load->model('localisation/geo_zone');

		$data['geo_zones'] = $this->model_localisation_geo_zone->getGeoZones();

		$data['payment_bank_transfer_status'] = $this->configGet('payment_bank_transfer_status');
		$data['payment_bank_transfer_sort_order'] = $this->configGet('payment_bank_transfer_sort_order');
	...
	public function save(): void {
		...
		if (!$json) {
			...
			$this->model_setting_setting->editSetting('payment_bank_transfer', $this->request->post,$store_ID);
	...
	protected function configGet($searched='')
	{
		...
		if ($this->store_info) {
			...
			$this->store_info = $this->model_setting_setting->getSetting('payment_bank_transfer', $this->store_ID );
			return isset($this->store_info[$searched])?$this->store_info[$searched]:'';
		}
	}
}
