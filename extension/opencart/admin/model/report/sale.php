...
class Sale extends \Opencart\System\Engine\Model {
	public function getTotalSales(array $data = []): float {
		...
		$sql = "SELECT SUM(`total`) AS `total` FROM `" . DB_PREFIX . "order` WHERE `order_status_id` > '0' AND store_id='".(int)$store_id."'";
	...
	public function getTotalOrdersByCountry(): array {
		$store_id = isset($this->session->data['store_id'])?$this->session->data['store_id']:0;
		$query = $this->db->query("SELECT COUNT(*) AS total, SUM(o.`total`) AS amount, c.`iso_code_2` FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "country` c ON (o.`payment_country_id` = c.`country_id`) WHERE o.`order_status_id` > '0' AND o.store_id='".(int)$store_id."' GROUP BY o.`payment_country_id`");
	...
	public function getTotalOrdersByDay(): array {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, HOUR(`date_added`) AS hour FROM `" . DB_PREFIX . "order` WHERE `order_status_id` IN(" . implode(",", $implode) . ") AND store_id='".(int)$store_id."' ORDER AND DATE(`date_added`) = DATE(NOW()) GROUP BY HOUR(`date_added`) BY `date_added` ASC");
	...
	public function getTotalOrdersByWeek(): array {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, `date_added` FROM `" . DB_PREFIX . "order` WHERE `order_status_id` IN(" . implode(",", $implode) . ") AND store_id='".(int)$store_id."' AND DATE(`date_added`) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') GROUP BY DAYNAME(`date_added`)");
	...
	public function getTotalOrdersByMonth(): array {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, `date_added` FROM `" . DB_PREFIX . "order` WHERE `order_status_id` IN(" . implode(",", $implode) . ") AND store_id='".(int)$store_id."' AND DATE(`date_added`) >= DATE('" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "') GROUP BY DATE(`date_added`)");
	...
	public function getTotalOrdersByYear(): array {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, `date_added` FROM `" . DB_PREFIX . "order` WHERE `order_status_id` IN(" . implode(",", $implode) . ") AND store_id='".(int)$store_id."' AND YEAR(`date_added`) = YEAR(NOW()) GROUP BY MONTH(`date_added`)");
