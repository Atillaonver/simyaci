...
class AuthorizeNetAim extends \Opencart\System\Engine\Controller {
	...
	public function index() {
		...
		for ($i = $today['year']; $i < $today['year'] + 11; $i++) {
			$data['year_expire'][] = array(
				...
				'value' => substr(date('Y', mktime(0, 0, 0, 1, 1, $i)), -2)
			);
		}
		
		$data['language'] = $this->config->get('config_language');

		return $this->load->view('extension/bytao/payment/authorizenet_aim', $data);
	}
	...
	public function send() {
		...
		if (curl_error($curl)) {
			...
			$this->log->write('AUTHNET AIM CURL ERROR: ' . curl_errno($curl) . '::' . curl_error($curl),1);
		} elseif ($response) {
			$i = 1;
			...
			if ($response_info[1] == '1') {
				...
				$this->log->write('Authorize.net debug (Response): ' . $message,1);
			...
			$this->log->write('AUTHNET AIM CURL ERROR: Empty Gateway Response',1);
		}
