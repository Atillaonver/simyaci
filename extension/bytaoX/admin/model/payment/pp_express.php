<?php
namespace Opencart\Admin\Model\Extension\Bytao\Payment;
class PpExpress extends \Opencart\System\Engine\Model {
	...
	public function call($data) {
		...
		if ($this->config->get('pp_express_test') == 1) {
			...
			$user = $this->config->get('pp_express_sandbox_username'.(int)$this->session->data['store_id']);
			$password = $this->config->get('pp_express_sandbox_password'.(int)$this->session->data['store_id']);
			$signature = $this->config->get('pp_express_sandbox_signature'.(int)$this->session->data['store_id']);
		} else {
			$api_endpoint = 'https://api-3t.paypal.com/nvp';
			$user = $this->config->get('pp_express_username'.(int)$this->session->data['store_id']);
			$password = $this->config->get('pp_express_password'.(int)$this->session->data['store_id']);
			$signature = $this->config->get('pp_express_signature'.(int)$this->session->data['store_id']);
		}
