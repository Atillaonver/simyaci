...
class Transaction {
    ...
    public function addTransaction($array, $api_name) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "paytr_" . $this->db->escape($api_name) . "_transaction SET 
            order_id = '" . (int)$array['order_id'] . "',
            merchant_oid = '" . $this->db->escape($array['merchant_oid']) . "',
            total = '" . $array['total'] . "',
            is_order = 1,
            is_complete = '" . (int)$array['is_complete'] . "',
            is_failed = '" . (int)$array['is_failed'] . "',
            is_refunded = 0,
            date_added = NOW()"
        );
    ...
    /**
     * Adds a transaction specifically for callback scenarios.
     */
    public function addTransactionForCallback($array, $api_name) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "paytr_" . $this->db->escape($api_name) . "_transaction SET 
            ...
            status_message = '" . $this->db->escape($array['status_message']) . "',
            is_order = 1,
            is_complete = '" . (int)$array['is_complete'] . "',
            is_failed = '" . (int)$array['is_failed'] . "',
            is_refunded = 0,
    ...
    /**
     * Updates transaction details for callback processing.
     */
    public function updateTransactionForCallback($array, $api_name) {
        $this->db->query("UPDATE " . DB_PREFIX . "paytr_" . $this->db->escape($api_name) . "_transaction SET 
            total_paid = '" . $array['total_paid'] . "',
            status = '" . $this->db->escape($array['status']) . "',
            status_message = '" . $this->db->escape($array['status_message']) . "',
            is_complete = 1,
            date_updated = NOW()
            WHERE merchant_oid = '" . $this->db->escape($array['merchant_oid']) . "'"
        );
    }

    /**
     * Updates transaction details for refund processing.
     */
    public function updateTransactionForRefund($array, $api_name) {
        $this->db->query("UPDATE " . DB_PREFIX . "paytr_" . $this->db->escape($api_name) . "_transaction SET 
            is_refunded = 1,
            refund_status = '" . $this->db->escape($array['refund_status']) . "',
            refund_amount = '" . $array['refund_amount'] . "',
            date_updated = NOW()
            WHERE merchant_oid = '" . $this->db->escape($array['merchant_oid']) . "'"
        );
    }
    ...
    /**
     * Updates transaction details for EFT notifications.
     */
    public function updateTransactionForEftNotify($array) {
        $this->db->query("UPDATE " . DB_PREFIX . "paytr_eft_transaction SET 
            status = '" . $this->db->escape($array['status']) . "',
            notify_message = '" . $this->db->escape($array['notify_message']) . "',
            is_notify = 1,
            date_updated = NOW()
            WHERE merchant_oid = '" . $this->db->escape($array['merchant_oid']) . "'"
        );
    ...
    /**
     * Retrieves a list of transactions for refund purposes.
     */
    public function getListTransactionsForRefund($order_id, $lang, $api_name) {
        ...
        if ($transactions) {
            ...
            foreach ($transactions as $transaction) {
                ...
                // Status Message
                $content['status_message'] = ($transaction['status_message'] == 'completed')
                    ? '<span class="label label-success">' . $lang->get('text_refund_completed') . '</span>'
                    : '<span class="label label-danger">' . $transaction['status_message'] . '</span>';
                ...
                // Refunded?
                $content['is_refunded'] = $transaction['is_refunded']
                    ? $lang->get('text_refund_yes')
                    : $lang->get('text_refund_no');
                ...
                if ($transaction['total_paid'] >= 1 && $transaction['refund_status'] != 'full' && $transaction['status_message'] == 'completed') {
                    ...
                    $amount = ($transaction['refund_amount'] < $transaction['total'])
                        ? $transaction['total'] - $transaction['refund_amount']
                        : $transaction['total'];
    ...
    /**
     * Retrieves a transaction by its merchant OID.
     */
    public function getTransactionByMerchantOID($merchant_oid, $api_name, $fetch_all = false) {
        ...
        return $fetch_all ? $query->rows : $query->row;
    }

    /**
     * Retrieves a transaction by its merchant OID, excluding failed transactions.
     */
    public function getTransactionByMerchantOIDByFailed($merchant_oid, $api_name, $fetch_all = false) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "paytr_" . $this->db->escape($api_name) . "_transaction WHERE merchant_oid = '" . $this->db->escape($merchant_oid) . "' AND is_failed = 0 AND is_complete = 1");

        return $fetch_all ? $query->rows : $query->row;
    }

    /**
     * Retrieves transactions by order ID.
     */
