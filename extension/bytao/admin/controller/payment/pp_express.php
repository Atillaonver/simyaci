...
class PPExpress extends \Opencart\System\Engine\Controller {

	public function index() {
		$this->load->language('extension/bytao/payment/pp_express');
		$this->document->setTitle($this->language->get('heading_title'));
		
		$data['save'] = $this->url->link('extension/bytao/payment/pp_express.save', 'user_token=' . $this->session->data['user_token']);
		$data['back'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=payment');


		$this->load->model('setting/setting');
		$this->load->model('setting/store');
		$this->load->model('extension/bytao/payment/pp_express');
		$this->load->model('tool/image');
		
		$data['store_id'] = $store_id = isset($this->session->data['store_id'])?$this->session->data['store_id']:0; 
		$setting = $this->model_setting_setting->getSetting('payment_pp_express', $store_id);
		
		
		$data['payment_pp_express_username'] =  isset($setting['payment_pp_express_username'])?$setting['payment_pp_express_username']:'';
		$data['payment_pp_express_password']= isset($setting['payment_pp_express_password'])?$setting['payment_pp_express_password']:'';
		$data['payment_pp_express_signature'] = isset($setting['payment_pp_express_signature'])?$setting['payment_pp_express_signature']:'';
		$data['payment_pp_express_logo'] = isset($setting['payment_pp_express_logo'])?$setting['payment_pp_express_logo']:'';
		
		if ($data['payment_pp_express_logo'] && is_file(DIR_IMAGE . $data['payment_pp_express_logo'])) {
				$data['thumb'] = $this->model_tool_image->resize($data['payment_pp_express_logo'], 750, 90);
		...
		$data['payment_pp_express_ipn_url'] = isset($setting['payment_pp_express_ipn_url'])?$setting['payment_pp_express_ipn_url']:'index.php?route=extention/bytao/payment/pp_express/ipn';
		
		$data['payment_pp_express_sandbox_username'] = isset($setting['payment_pp_express_sandbox_username'])?$setting['payment_pp_express_sandbox_username']:'';
		
		$data['payment_pp_express_sandbox_password'] = isset($setting['payment_pp_express_sandbox_password'])?$setting['payment_pp_express_sandbox_password']:'';
		
		$data['payment_pp_express_sandbox_signature'] = isset($setting['payment_pp_express_sandbox_signature'])?$setting['payment_pp_express_sandbox_signature']:'';
		
		$data['payment_pp_express_test'] = isset($setting['payment_pp_express_test'])?$setting['payment_pp_express_test']:'';;
		
		$data['payment_pp_express_method'] = isset($setting['payment_pp_express_method'])?$setting['payment_pp_express_method']:'';
		
		$data['payment_pp_express_total'] = isset($setting['payment_pp_express_total'])?$setting['payment_pp_express_total']:'';
		
		
		$data['payment_pp_express_debug'] = isset($setting['payment_pp_express_debug'])?$setting['payment_pp_express_debug']:'';
		
		$data['payment_pp_express_currency'] = isset($setting['payment_pp_express_currency'])?$setting['payment_pp_express_currency']:'USD';
		
		$data['currency_codes'] = $this->model_extension_bytao_payment_pp_express->currencyCodes();

		$this->load->model('localisation/order_status');

		$data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();

		$data['payment_pp_express_canceled_reversal_status_id'] =  isset($setting['payment_pp_express_canceled_reversal_status_id'])?$setting['payment_pp_express_canceled_reversal_status_id']:'';
		
		$data['payment_pp_express_completed_status_id'] =  isset($setting['payment_pp_express_completed_status_id'])?$setting['payment_pp_express_completed_status_id']:'';
		
		$data['payment_pp_express_denied_status_id'] =  isset($setting['payment_pp_express_denied_status_id'])?$setting['payment_pp_express_denied_status_id']:'';
		
		$data['payment_pp_express_expired_status_id'] =  isset($setting['payment_pp_express_expired_status_id'])?$setting['payment_pp_express_expired_status_id']:'';
				
		$data['payment_pp_express_failed_status_id'] =  isset($setting['payment_pp_express_failed_status_id'])?$setting['payment_pp_express_failed_status_id']:'';
		
		$data['payment_pp_express_pending_status_id'] =  isset($setting['payment_pp_express_pending_status_id'])?$setting['payment_pp_express_pending_status_id']:'';
		
		$data['payment_pp_express_processed_status_id'] =  isset($setting['payment_pp_express_processed_status_id'])?$setting['payment_pp_express_processed_status_id']:'';
		
		$data['payment_pp_express_refunded_status_id'] =  isset($setting['payment_pp_express_refunded_status_id'])?$setting['payment_pp_express_refunded_status_id']:'';
		
		$data['payment_pp_express_reversed_status_id'] =  isset($setting['payment_pp_express_reversed_status_id'])?$setting['payment_pp_express_reversed_status_id']:'';
		
		$data['payment_pp_express_voided_status_id'] =  isset($setting['payment_pp_express_voided_status_id'])?$setting['payment_pp_express_voided_status_id']:'';
		
		$data['payment_pp_express_allow_note'] =  isset($setting['payment_pp_express_allow_note'])?$setting['payment_pp_express_allow_note']:'';
		
		$data['payment_pp_express_page_colour'] =  isset($setting['payment_pp_express_page_colour'])?str_replace('#', '', $setting['payment_pp_express_page_colour']):'';
		
		$data['payment_pp_express_recurring_cancel_status'] = isset($setting['payment_pp_express_recurring_cancel_status'])?$setting['payment_pp_express_recurring_cancel_status']:'';
		...
		$data['text_paypal_link'] = 'https://www.paypal.com/webapps/merchantboarding/webflow/externalpartnerflow?'
				...
				. '&returnToPartnerUrl=' . base64_encode(html_entity_decode($this->url->link('payment/pp_express/live', 'user_token=' . $this->session->data['user_token'])))
		...
		$data['text_paypal_link_sandbox'] = 'https://www.sandbox.paypal.com/webapps/merchantboarding/webflow/externalpartnerflow?'
				...
				. '&productIntentID=addipmt'
				. '&receiveCredentials=TRUE'
				. '&returnToPartnerUrl=' . base64_encode(html_entity_decode($this->url->link('payment/pp_express/sandbox', 'user_token=' . $this->session->data['user_token'])))
				. '&subIntegrationType=S';
		...
		$this->response->setOutput($this->load->view('extension/bytao/payment/pp_express', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/bytao/payment/pp_express')) {
			$json['error']['warning'] = $this->language->get('error_permission');
		}

		if (empty($this->request->post['payment_pp_express_username'])) {
				$json['error']['payment_pp_express_username'] = $this->language->get('error_username');
		}
		
		if (empty($this->request->post['payment_pp_express_password'])) {
				$json['error']['payment_pp_express_password'] = $this->language->get('error_key');
		}
		
		if (empty($this->request->post['payment_pp_express_signature'])) {
				$json['error']['payment_pp_express_signature'] = $this->language->get('error_key');
		}
		/*
		if (empty($this->request->post['payment_pp_express_ipn_url'])) {
				$json['error']['payment_pp_express_ipn_url'] = $this->language->get('error_key');
		}
		*/
		
		if (!$json) {
			$this->load->model('setting/setting');
			$this->model_setting_setting->editSetting('payment_pp_express', $this->request->post,$store_id);
	...
	public function refund() {
		...
		$this->load->language('extension/bytao/payment/pp_express_refund');
	...
	public function orderAction() {
		if ($this->config->get('payment_pp_express_status')) {
			$this->load->model('payment/pp_express');
			$this->load->language('extension/bytao/payment/pp_express_order');
	...
	public function search() {
		...
		$this->load->language('extension/bytao/payment/pp_express_search');
		$this->load->model('payment/pp_express');
	...
	public function viewTransaction() {
		$this->load->model('payment/pp_express');
		$this->load->language('extension/bytao/payment/pp_express_view');
