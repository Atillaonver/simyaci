<?php
namespace Opencart\Catalog\Controller\Extension\Bytao\Payment;
class PpExpress extends \Opencart\System\Engine\Controller {
	
	public function index() {
		$this->language->load('extension/bytao/payment/pp_express');

		$data['button_continue'] = $this->language->get('button_continue');
		$data['button_continue_action'] = $this->url->link('tool/pp.checkout','language='.$this->config->get('config_language'));

		/**
		 * if there is any other paypal session data, clear it
		 */
		unset($this->session->data['paypal']);
		return $this->load->view('extension/bytao/payment/pp_express', $data);
	}
	...
	public function express() {
		...
		$data = array(
			...
			'MAXAMT'             => $max_amount,
			'RETURNURL'          => $this->url->link('extension/bytao/payment/pp_express/expressReturn', 'language='.$this->config->get('config_language')),
			...
			'LOCALECODE'         => 'EN',
			'LANDINGPAGE'        => 'Login',
			'HDRIMG'             => $this->model_tool_image->resize($this->config->get('payment_pp_express_logo'), 790, 90),
			'PAYFLOWCOLOR'       => $this->config->get('payment_pp_express_page_colour'),
		...
		$data = array_merge($data, $this->model_extension_bytao_payment_paypal->paymentRequestInfo());

		$result = $this->model_extension_bytao_payment_paypal->call($data);
		...
		if (!isset($result['TOKEN'])) {
			 ...
			 */
			if ($this->config->get('payment_pp_express_debug') == 1) {
				$this->log->write(serialize($result),1);
			}
			$this->response->redirect($this->url->link('checkout/checkout', 'language='.$this->config->get('config_language')));
		}
	...
	public function expressReturn() {
		...
		$this->response->redirect($this->url->link('extension/bytao/payment/pp_express/expressConfirm', 'language='.$this->config->get('config_language')));
	}
	...
	public function expressConfirm() {
		...
		$data['breadcrumbs'][] = array(
			'href' => $this->url->link('extension/bytao/payment/pp_express/express'),
			'text' => $this->language->get('text_title')
		);

		$data['breadcrumbs'][] = array(
			'href' => $this->url->link('extension/bytao/payment/pp_express/expressConfirm'),
		...
		$data['action'] = $this->url->link('extension/bytao/payment/pp_express/expressConfirm', 'language='.$this->config->get('config_language'));
			...
			if (!empty($shipping_address)) {
				...
				if (!empty($results)) {
					...
					if (!empty($quote_data)) {
						...
						$data['action_shipping'] = $this->url->link('extension/bytao/payment/pp_express/shipping', 'language='.$this->config->get('config_language'));
					} else {
						unset($this->session->data['shipping_methods']);
						unset($this->session->data['shipping_method']);
		...
		$data['action_confirm'] = $this->url->link('extension/bytao/payment/pp_express/expressComplete', 'language='.$this->config->get('config_language'));
		...
		$this->response->setOutput($this->load->view('extension/bytao/payment/pp_express_confirm', $data));
	...
	public function expressComplete() {
		...
		if ($redirect == '') {
			...
			$this->language->load('checkout/checkout','language='.$this->config->get('config_language'));
			...
			$this->load->model('extensiın/bytao/payment/pp_express');

			$paypal_data = array(
				'TOKEN'                      => $this->session->data['paypal']['token'],
				'PAYERID'                    => $this->session->data['paypal']['payerid'],
				'METHOD'                     => 'DoExpressCheckoutPayment',
				'PAYMENTREQUEST_0_NOTIFYURL' => $this->url->link('extension/bytao/payment/pp_express/ipn', 'language='.$this->config->get('config_language')),
				'RETURNFMFDETAILS'           => 1
			);

			$paypal_data = array_merge($paypal_data, $this->model_extension_bytao_payment_paypal->paymentRequestInfo());

			$result = $this->model_extension_bytao_payment_paypal->call($paypal_data);
			...
			if ($result['ACK'] == 'Success') {
				...
				$paypal_order_data = array(
					'order_id'         => $order_id,
					'capture_status'   => ($this->config->get('payment_pp_express_method') == 'Sale' ? 'Complete' : 'NotComplete'),
				...
				$paypal_order_id = $this->model_extension_bytao_payment_paypal->addOrder($paypal_order_data);
				...
				$paypal_transaction_data = array(
					...
					'transaction_entity'    => ($this->config->get('payment_pp_express_method') == 'Sale' ? 'payment' : 'auth'),
					'amount'                => $result['PAYMENTINFO_0_AMT'],
					'debug_data'            => json_encode($result)
				);

				$this->model_extension_bytao_payment_paypal->addTransaction($paypal_transaction_data);
	...
	public function checkout() {
		...
		$check = $this->url->link('checkout/checkout', 'language='.$this->config->get('config_language'));
		
		$data = [
			'METHOD'             => 'SetExpressCheckout',
			'MAXAMT'             => $max_amount,
			'RETURNURL'          => $this->url->link('extension/bytao/payment/pp_express/checkoutReturn', 'language='.$this->config->get('config_language')),
			'CANCELURL'          => $check,
			'REQCONFIRMSHIPPING' => 0,
			'NOSHIPPING'         => $shipping,
			'LOCALECODE'         => 'EN',
	...
	public function checkoutReturn() {
		$this->log->write('checkoutReturn');	
		$this->language->load('extension/bytao/payment/pp_express');
		/**
		 * Get the details
		 */
		 
		$this->load->model('model/extension/bytao/payment/paypal');
		$this->load->model('checkout/order');
		...
		if ($result['ACK'] == 'Success') {
			...
			$this->response->setOutput($this->load->view('extension/bytao/payment/not_found'));
	...
	public function ipn() {
		...
		if (!$response) {
			$this->model_extension_bytao_payment_paypal->log(array('error' => curl_error($curl),'error_no' => curl_errno($curl)), 'Curl failed');
		}

		$this->model_extension_bytao_payment_paypal->log(array('request' => $request,'response' => $response), 'IPN data');
		...
		if ((string)$response == "VERIFIED")  {
			...
			if (isset($this->request->post['txn_id'])) {
				$transaction = $this->model_extension_bytao_payment_paypal->getTransactionRow($this->request->post['txn_id']);
			} else {
				$transaction = false;
			}

			if (isset($this->request->post['parent_txn_id'])) {
				$parent_transaction = $this->model_extension_bytao_payment_paypal->getTransactionRow($this->request->post['parent_txn_id']);
			} else {
			...
			if ($transaction) {
				...
				if ($parent_transaction) {
					...
					$this->model_extension_bytao_payment_pp_express->addTransaction($transaction);
					...
					if (isset($this->request->post['auth_status']) && $this->request->post['auth_status'] == 'Completed' && $parent_transaction['payment_status'] == 'Pending') {
						$captured = $this->currency->format($this->model_extension_bytao_payment_paypal->totalCaptured($parent_transaction['paypal_order_id']), false, false, false);
						$refunded = $this->currency->format($this->model_extension_bytao_payment_paypal->totalRefundedOrder($parent_transaction['paypal_order_id']), false, false, false);
						...
						$this->model_extension_bytao_payment_pp_express->updateOrder('Complete', $parent_transaction['order_id']);
					}
			...
			$this->model_extension_bytao_payment_paypal->log(array('IPN was invalid'), 'IPN fail');
		} else {
			if ($this->config->get('payment_pp_express_debug') == 1) {
	...
	public function recurringCancel() {
		...
		$this->log->write('recurringCancel');	
		...
		if ($recurring && !empty($recurring['reference'])) {
			...
			$result = $this->model_extension_bytao_payment_paypal->recurringCancel($recurring['reference']);
		...
		$this->response->redirect($this->url->link('account/recurring/info', 'recurring_id=' . $this->request->get['recurring_id'].'&language='.$this->config->get('config_language')));
	}
	...
	public function recurringButtons() {
		...
		if ($recurring['status'] == 2 || $recurring['status'] == 3) {
			$data['buttons'][] = array(
				...
				'link' => $this->url->link('extension/bytao/payment/pp_express/recurringCancel', 'recurring_id=' . $this->request->get['recurring_id'].'&language='.$this->config->get('config_language'))
			);
		}

		$data['buttons'][] = array(
			'text' => $this->language->get('button_continue'),
			'link' => $this->url->link('account/recurring', 'language='.$this->config->get('config_language'))
		);
