...
class iyzicox extends Controller
{
    private $error = [];
    private $iyzico;
    ...
    public function index(): void
    {
		...
		$this->log->write(' URL :'.print_r($this->store_info,true));
    ...
    private function setWebookUrl(): bool
    {
		...
		if (!$getWebhookUrlKey){
			$this->model_setting_setting->editSetting('webhook_iyzico', ["webhook_iyzico_webhook_url_key" => $generateUrlId],$this->store_ID );
			$this->store_info['webhook_iyzico_webhook_url_key']= $generateUrlId;
		}
        return true;
    }
    ...
    private function setWebhookUpdate(): void
    {
        ...
        $configWebhookStatus = $this->configGet('payment_iyzico_webhook_active_button');
        $configApikey = $this->configGet('payment_iyzico_api_key');
        $configSecretKey = $this->configGet('payment_iyzico_secret_key');

        if (isset($configApikey) && isset($configSecretKey)) {
            if ($configWebhookStatus == 1) {
                $webhookPost = new stdClass();
				$webhookPost->webhookUrl = ($this->store_ID?$this->store_info['config_url']:HTTP_CATALOG) . 'index.php?route=extension/payment/iyzico.webhook&key=' . $this->configGet('webhook_iyzico_webhook_url_key');
                ...
                if(isset($requestResponseWebhook->merchantNotificationUpdateStatus)){
                    if ($requestResponseWebhook->merchantNotificationUpdateStatus == 'UPDATED' || $requestResponseWebhook->merchantNotificationUpdateStatus == 'CREATED')
                        $this->model_setting_setting->editSetting('payment_iyzico_webhook', array("payment_iyzico_webhook_active_button" => 2),$this->store_ID);
					else
						$this->model_setting_setting->editSetting('payment_iyzico_webhook', array("payment_iyzico_webhook_active_button" => 3),$this->store_ID);
                }
            }
        }
    }
